...
async function main () {
  ...
  try {
    const store = make()
    const blank = table('|  |  |  |  |')
    check('blank scaffold is not learned', (await store.sync('Greek/Vocabulary.lang', blank)).length === 0)

    const first = table(
      '| νερό | water | Θέλω νερό. |  |',
      '| ψωμί | bread |  |  |'
    )
    const created = await store.sync('Greek/Vocabulary.lang', first)
    ...
    clock = 2_000
    const edited = await store.sync('Greek/Vocabulary.lang', table(
    ...
    clock = 3_000
    const moved = await store.sync('Greek/Vocabulary.lang', table(
      '| ψωμί | bread |  |  |',
      '| νερό | water | Πίνω νερό. | common |'
    ))
    ...
    const shifted = await store.sync('Greek/Vocabulary.lang', withIncomplete)
    ...
    check('folder moves carry row history', (await store.rows('Languages/Greek/Vocabulary.lang')).length === 2)
    check('old paths are removed after a move', (await store.rows('Greek/Vocabulary.lang')).length === 0)

    const reloaded = make()
    const persisted = await reloaded.rows('Languages/Greek/Vocabulary.lang')
    check('history survives a fresh store', persisted[1].id === 'row-1' && persisted[1].addedAt === 1_000)

    clock = 4_000
    const baseline = await reloaded.sync(
      'Languages/Greek/Older Vocabulary.lang',
    ...
    check('folder deletion removes its history', (await reloaded.rows('Languages/Greek/Vocabulary.lang')).length === 0)
    ...
    const note = 'Race/Vocabulary.lang'
    const onDisk = table(
      '| νερό | water | Θέλω νερό. |  |',
