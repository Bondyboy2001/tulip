...
/**
 * The table's rows as a plain grid, header first — the inverse of
 * `importCards`.
 *
 * Read by the same `cells`/`delimiter` rules the cards are, so what leaves is
 * exactly what studied; the CSV dialect (quoting, delimiters) belongs to
 * src/csv.js's `formatSeparated`, which the caller reaches for. Answers with
 * null when the note holds no table. Rows short of the header's width are
 * padded so the grid is rectangular for whoever writes it.
 */
export function exportCards (markdown) {
  const lines = String(markdown || '').split(/\r?\n/)
  for (let at = 0; at < lines.length - 1; at++) {
    if (!lines[at].includes('|') || !delimiter(lines[at + 1])) continue
    const header = cells(lines[at])
    const rows = []
    for (let row = at + 2; row < lines.length && lines[row].includes('|'); row++) {
...
/* The queue's rules — what is due, what is unlocked, how many new cards a day —
   live in review-queue.js, shared with the flashcard bank's study. */
export {
  UNLOCK_STABILITY, unlocked, NEW_PER_DAY, buildQueue, dueCount
} from './review-queue.js'
import { buildQueue, NEW_PER_DAY } from './review-queue.js'
