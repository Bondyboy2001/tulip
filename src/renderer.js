...
import {
  ...
  isCodePath, isDataPath, isNotebookPath, isDocxPath, isFlashcardBankPath,
  isLanguageTablePath, codeToken,
  isViewedFilePath
} from './vault-paths.js'
...
/* `el` is this module's own name for the DOM registry, so blocks.js's element
   ...
   do. */
import { codeCopilotButton, codeCopyButton } from './blocks.js'
import { svgIcon, el as node, guardModalBackground, reason, trapModalFocus } from './dom.js'
import { keyLabel, localiseShortcuts, revealLabel } from './platform.js'
import { loadFeatureStyles } from './lazy-styles.js'
import { fileIcon } from './file-icons.js'
import {
  enhanceFlashcards, flashcardMarkdown, parseFlashcards, flashcardTags,
  flashcardCards
} from './flashcards.js'
import { buildQueue } from './review-queue.js'
import { AGAIN, GOOD, grade as gradeCard } from './srs.js'
import { makeReviewSink } from './review-sink.js'
import { makeLineScanner } from './spelling.js'
import { prepareMath, equationIndex, equationsFor, docText } from './math.js'
import { dressCitations, parseBibTeXCached, citationLabel } from './citations.js'
...
import {
  initTransclusion, refreshTransclusions, installNotePreview, standsAlone
} from './transclude.js'
import {
...
import { fuzzy, matchItem, markHits, markWords } from './match.js'
...
import {
  ...
  resolveRunNote, resolveRunPermission, retirePainters, runBlocksInOrder
} from './runcode.js'
...
const el = {
  ...
  viewAnnounce: /** @type {HTMLElement} */ ($('view-announce')),
  ...
  fcStudyClose: /** @type {HTMLButtonElement} */ ($('fc-study-close')),
  fcStudyTags: /** @type {HTMLDivElement} */ ($('fc-study-tags')),
  fcStudyCard: /** @type {HTMLElement} */ ($('fc-study-card')),
  fcStudyImage: /** @type {HTMLImageElement} */ ($('fc-study-image')),
  fcStudyQuestion: /** @type {HTMLHeadingElement} */ ($('fc-study-question')),
  fcStudyOptions: /** @type {HTMLDivElement} */ ($('fc-study-options')),
  fcStudyFeedback: /** @type {HTMLParagraphElement} */ ($('fc-study-feedback')),
  fcStudyExplanation: /** @type {HTMLParagraphElement} */ ($('fc-study-explanation')),
  fcStudyNext: /** @type {HTMLButtonElement} */ ($('fc-study-next')),
  ...
  outlineFoldAll: /** @type {HTMLButtonElement} */ ($('btn-outline-fold-all')),
...
const viewingFlashcardBank = () => state.current?.kind === 'flashcards'
...
function noteRef (path) {
  ...
  const flashcards = isFlashcardBankPath(path)
              ...
              : isNotebookPath(path)
                  ...
                  : (isCodePath(path) || probedText.has(path))
                      ...
                      : isViewedFilePath(path)
                          ? 'file'
                          : (flashcards ? 'flashcards' : language ? 'language' : 'note')
  }
}
...
const editorDeps = {
  ...
  noteNames: () => vaultLists().noteNames,
  ...
  embedChoices: () => vaultLists().embedChoices,
...
/* The two whole-vault lists the completion sources ask for.

   Both used to be rebuilt from scratch on the way in: `noteNames` on every
   keystroke inside `[[`, `embedChoices` on every document or selection change
   once a line contains `![[`. Each mapped and filtered the entire file list and
   the entire attachment list to produce something that changes only when the
   vault does.

   Keyed on the identity of the two arrays, which are replaced wholesale when
   the snapshot brings a new tree or a new attachment list — so this cannot go
   stale, and a file added a moment ago is still offered at once. */
let vaultListCache = { files: null, assets: null, noteNames: [], embedChoices: [] }

function vaultLists () {
  if (vaultListCache.files === state.files && vaultListCache.assets === state.assets) {
    return vaultListCache
  }
  const noteNames = state.files.map((f) => ({ name: f.name, dir: f.dir }))
  const embedChoices = state.assets.map((path) => ({ label: path, name: path }))
  for (const path of state.assets) {
    if (!['audio', 'video'].includes(assetKind(path))) continue
    noteNames.push({
      name: path.split('/').pop(),
      dir: path.split('/').slice(0, -1).join('/'),
      detail: 'media'
  ...
  for (const f of state.files) {
    if (NOTE_EXT.test(f.path)) embedChoices.push({ label: f.name, name: f.name })
  }
  vaultListCache = { files: state.files, assets: state.assets, noteNames, embedChoices }
  return vaultListCache
}
...
/** How many lines the note has, by the same two routes — for clamping a line
 *  number that came from a search hit or the copilot before going to it. */
function noteLines () {
  if (editor) return editor.state.doc.lines
  let lines = 1
  for (let i = buffer.text.indexOf('\n'); i >= 0; i = buffer.text.indexOf('\n', i + 1)) lines++
  return lines
}
...
/* The PDF chrome is in the shell, but its viewer, find bar and stylesheet are
   not on the Markdown launch path. Both the ordinary reader and TeX preview
   meet here, so whichever arrives first builds the two small viewer instances
   once and every later call is synchronous. */
function ensurePdf () {
  ...
  pdfLoading ||= Promise.all([
    ...
    pdf = mountPdf({
      ...
      onNote: (mark) => openOverlay('append-to', { mark })
    })
...
async function compileTex (generation = ++texCompileGeneration) {
  ...
  try {
    ...
    toast(reason(err, 'The compiled PDF could not be opened.'))
  }
}
...
/* Document viewers own their buffers, but the shell owns the shared dirty
   ...
   a viewer clean: only its next `onDirty` report can say that. */
...
function ensureNotebook () {
  ...
  if (!notebookLoading) {
    notebookLoading = Promise.all([
      import('./notebook.js'),
      loadFeatureStyles('notebook')
    ]).then(([{ mountNotebook }]) => {
...
function paintSiteBar (view) {
  ...
  el.siteExternal.disabled = !view.url
  paintSiteMark(view)
}

/**
 * The marker at the head of the address bar.
 *
 * Two questions, answered in one place because they occupy one slot: whose
 * page this is, and whether the connection is one anybody in between can read.
 * A secure page shows the site's own icon — which is what a reader recognises
 * a site by — and falls back to a plain padlock while the icon is on its way
 * or the site has none. Plain http shows words rather than a symbol, because
 * "not secure" is the one thing here that a glyph has never managed to say.
 */
/* What the marker is currently drawn from. The bar repaints on every state
   change a page reports — a title, a navigation, each end of a load — and
   rebuilding the <img> each time asked the site for its icon again on every
   one of them. The two facts it is made of are cheap to compare; the icon is
   not cheap to fetch. */
let siteMarkShows = { secure: '', favicon: '' }

function paintSiteMark (view) {
  const mark = el.siteMark
  if (siteMarkShows.secure === view.secure && siteMarkShows.favicon === view.favicon) return
  siteMarkShows = { secure: view.secure, favicon: view.favicon }

  mark.className = `site-mark is-${view.secure || 'none'}`
  mark.hidden = !view.secure
  mark.replaceChildren()
  if (!view.secure) return

  if (view.secure === 'plain') {
    mark.title = 'This page is not sent securely; anyone on this network can read it.'
    mark.textContent = 'Not secure'
    return
  }
  if (view.secure === 'local') {
    mark.title = 'A server on this computer. Nothing leaves the machine.'
    mark.textContent = 'Local'
    return
  }

  mark.title = 'The connection to this site is encrypted.'
  if (view.favicon) {
    /* The icon is fetched by the app's own page rather than by the guest, so
       it is one request made outside the fence — to a host the reader is
       already looking at, over https, and for an image the page's CSP is the
       only thing it is allowed to be. A site that will not serve it falls back
       to the padlock rather than leaving a broken frame in the bar. */
    const icon = document.createElement('img')
    icon.className = 'site-favicon'
    icon.alt = ''
    icon.referrerPolicy = 'no-referrer'
    icon.src = view.favicon
    icon.addEventListener('error', () => {
      if (!icon.isConnected) return
      icon.replaceWith(sitePadlock())
      // So a later repaint does not put the broken icon back.
      siteMarkShows = { secure: view.secure, favicon: '' }
    })
    mark.append(icon)
    return
  }
  mark.append(sitePadlock())
}

const sitePadlock = () => svgIcon(
  '<path d="M4.6 7.2V5.4a3.4 3.4 0 0 1 6.8 0v1.8"/><rect x="3.2" y="7.2" width="9.6" height="6.4" rx="1.5"/>',
  { stroke: 1.3 })
...
function paintTools () {
  ...
  const pen = pdf?.pen() || MARK_COLORS[0].id
...
el.pdfPrev.addEventListener('click', () => { if (pdf) pdf.goToPage(pdf.page() - 1) })
el.pdfNext.addEventListener('click', () => { if (pdf) pdf.goToPage(pdf.page() + 1) })
...
/* Every address the vault already keeps, offered under the bar.
 *
 * Gathered when the field is focused rather than held up to date: a vault has
 * a handful of `.website` files and each is one line, so this is cheaper than
 * watching them — and it is the one moment the list is about to be read. The
 * held copy is only to keep ⌘L twice in a row from walking the disk twice. */
/* Which vault, and which version of its tree, the list on screen was built
   from. Keyed on the revision rather than on a clock: the answer can only
...
async function paintKnownSites () {
  ...
  for (const said of read) {
    if (!said?.url || seen.has(said.url)) continue
    seen.set(said.url, said.title || '')
  }
  el.siteKnown.replaceChildren(...[...seen].map(([url, title]) => {
    const option = document.createElement('option')
    option.value = url
    // The name the file carries, so the list reads as the sites you have
    // filed rather than as a column of URLs.
    if (title) option.label = title
    return option
  }))
}
...
el.siteAddress.addEventListener('focus', () => {
...
/* ---------------------------------------------------------------- drafts
   ...
   The policy — when to write a draft and when to throw one away — lives in
   src/drafts.js, beside its own reasoning. This is the wiring: what is open,
   the editor (built long after this line runs, so reached through a
   function), which documents a draft makes sense for, and the IPC calls. */
/* (`writeDraft` is the timer's callback; the renderer only queues and clears.) */
const { queueDraft, clearDraft } = makeDrafts({
  state,
  editor: () => editor,
  canDraft: (path) => !viewingPdf() && !viewingSite() && isEditableTextPath(path),
  save: (path, text) => api.draft.save(path, text),
  clear: (path) => api.draft.clear(path),
  docText
})
...
async function saveCurrentNow () {
  ...
  try {
    /* With the stamp of the version this buffer was read from, when it is
       still on hand. Main refuses the write if the file has moved on — the one
       way an outside edit inside the watcher's debounce could still be lost —
       and the refusal is handled exactly as a watcher event would have been:
       the merge panel, from the same base. */
    const expect = noteStamp.path === wrote ? noteStamp.stamp : null
    const wroteReply = await api.file.write(wrote, text, expect ? { expect } : null)
    if (wroteReply?.stale) {
      if (state.current?.path === wrote) await handleDiskConflict(wrote)
      return false
    }
    if (state.current?.path === wrote) holdNoteStamp(wrote, wroteReply?.stamp)
    /* The disk now holds `text`, so `text` is the version the buffer and the
       disk last agreed on — whether or not a keystroke landed mid-write. This
       used to be set only on the clean branch below, and on a large note a
       keystroke inside the write is the ordinary case: the base then stayed one
       save behind, and the next watcher event for the file — iCloud re-touching
       it after an upload, say — merged the buffer against the disk from a base
       that matched neither. Every such merge was a "conflict" whose other side
       was Tulip's own previous autosave, offered back as an outside edit. */
    {
      const tab = activeTab()
...
/* Saves can be triggered by typing, navigation, visibility changes and quit at
   the same time. One document write at a time prevents an older atomic rename
   from winning after a newer buffer has already been written. */
let saveInFlight = Promise.resolve(true)
function saveNow () {
  const next = saveInFlight.then(() => saveCurrentNow())
  saveInFlight = next.catch(() => false)
  return next
}
...
/* ------------------------------------------------------------- bookmark
   ...
   One place in a note to come back to — see src/bookmark.js for the marker
   and the ribbon both views draw for it. Setting one and going to one live
   here; opening a note at one is openText's. */
/**
 * Put the bookmark where the reader is: above the caret's line in the editing
 * view, above the line at the top of the page in the reading view. Any
 * bookmark the note already had goes in the same edit, so the change is one
 * undo and one save.
 */
async function setBookmark () {
  if (!state.current || !NOTE_EXT.test(state.current.path)) return
  /* Like linting: an edit, so it needs the editor that holds the undo
     history, whichever view the palette was opened from. */
  const wasReading = reading()
  const target = wasReading ? viewportLine() : 0
  const view = await ensureEditor()
  ...
  const edit = bookmarkInsertion(doc.toString(), at.number)
  /* The caret stays on its own text and the viewport stays where it was:
     forcing the selection to the bookmark's line scrolled the page out from
     under the reader on every placement. Let CodeMirror map the selection
     through the edit instead, and do not scroll to it. */
  if (edit.from !== edit.to || edit.insert) {
    view.dispatch({
      changes: { from: edit.from, to: edit.to, insert: edit.insert },
      scrollIntoView: false,
      userEvent: 'input.bookmark'
    })
  }
  saveNow()
...
/** Take the note to its bookmark — the line after the marker, in either view. */
function goToBookmark ({ quiet = false } = {}) {
  if (!state.current) return false
  const text = noteText()
  const marker = bookmarkLineOf(text)
  if (!marker) {
    if (!quiet) toast('This note has no bookmark. Set one with “Bookmark this place”.')
    return false
  }
  /* The caret goes to the line after the ribbon, where the writing resumes;
     the view is centred on the ribbon itself, so what was bookmarked is in
     the middle of the screen with its surroundings on both sides. */
  if (editor) {
    const { doc } = editor.state
    editor.dispatch({ selection: { anchor: doc.line(Math.min(marker + 1, doc.lines)).from } })
    if (!reading()) editor.scrollToLine(marker, { center: true })
  }
  if (reading()) restoreReadingPlace(marker, { center: true })
  return true
}
...
function wireTreeKeys () {
  el.tree.addEventListener('keydown', (event) => {
    ...
    /* The arrows carry the selection with them, the way a file manager's do:
       the row moved onto is the row selected, and holding shift extends from
       where the selection was last anchored instead of starting a new one.
       Without this the keyboard could reach every row and select none of them
       — and shift-↓, which is the first thing tried after shift-click, did
       nothing at all. */
    const goTo = (target, extend) => {
      if (!target) return
      const to = target.dataset.path
      /* A range measures from the anchor while it is still on screen; from the
         row the keys are on when it is not, which is what a first shift-↓ with
         nothing yet selected means. */
      const anchor = state.pickAnchor && visibleRows().includes(state.pickAnchor)
        ? state.pickAnchor
        : path
      if (!extend || !pickRange(anchor, to)) pickOnly(to)
      focusTreeRow(target)
    }

    const step = (by, extend) => {
      const rows = treeRows()
      const at = rows.indexOf(row)
      if (at === -1) return
      goTo(rows[Math.min(rows.length - 1, Math.max(0, at + by))], extend)
    }

    /* Everything the tree can reach. Scoped to the tree's own listener, so the
       editor's ⌘A still selects the text it is in. */
    if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 'a') {
      event.preventDefault()
      state.picked = new Set(visibleRows())
      state.pickAnchor = path
      markPicked()
      return
    }

    /* Escape gives a selection up, and says so by stopping there — otherwise
       the same press would also shut the narrow window's drawer, taking the
       tree away as the answer to "never mind". With nothing selected it is
       left alone to do exactly that. */
    if (event.key === 'Escape' && state.picked.size) {
      event.preventDefault()
    ...
    switch (event.key) {
      case 'ArrowDown': event.preventDefault(); step(1, event.shiftKey); return
      case 'ArrowUp': event.preventDefault(); step(-1, event.shiftKey); return
      case 'Home': {
        event.preventDefault()
        goTo(treeRows()[0], event.shiftKey)
        return
      }
      case 'End': {
        event.preventDefault()
        const rows = treeRows()
        goTo(rows[rows.length - 1], event.shiftKey)
        return
      }
      case 'ArrowRight': {
        event.preventDefault()
        /* Open, then — on a second press — step inside. A folder that is
           ...
           inside at all. Unfolding is not a move, so it leaves the selection
           where it is. */
      ...
      case 'ArrowLeft': {
        event.preventDefault()
        if (isFolder && open) { toggleFolder(path); focusTreePath(path); return }
        goTo(parentRowOf(path), event.shiftKey)
      }
    }
  })
}
...
function toggleAllFolders () {
  ...
  if (shutting) {
    /* Closing draws nothing: every open container empties and every open row
       drops its mark. Rebuilding here would construct a thousand rows — icons,
       listeners and all — only to put them behind folders that are shut. */
    for (const kids of el.tree.querySelectorAll('.children.is-open')) {
      kids.classList.remove('is-open')
      kids.replaceChildren()
    }
    for (const row of el.tree.querySelectorAll('.row.is-open')) {
      row.classList.remove('is-open')
      row.setAttribute('aria-expanded', 'false')
    }
    /* The invariants a rebuild would have left: one row still tabbable, the
       fold-all reading the tree it now governs. */
    restoreTreeTabStop()
    paintFoldToggle()
  } else {
    /* Opening is the reverse trade: every row does have to be drawn, which is
       what a rebuild is. */
    renderTree()
  }
...
function buildLevel (nodes, depth) {
  ...
  for (const node of nodes) {
    ...
    if (node.type === 'folder') {
      ...
      row.setAttribute('aria-selected', String(state.picked.has(node.path)))
      row.addEventListener('click', (e) => clickRow(node, e))
      /* Middle-click means the same thing a plain click does now — a tab of its
         own — and stays wired because it is the button people reach for. */
      row.addEventListener('auxclick', (e) => {
        if (e.button !== 1) return
        e.preventDefault()
        openFromTree(node.path).then(() => revealInTree(node.path))
      })
      frag.append(row)
    }
    ...
    row.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Enter') {
        e.preventDefault()
        node.type === 'folder' ? toggleFolder(node.path) : openFromTree(node.path)
        return
      }
...
function topLevelOnly (paths) {
  /* By ancestry rather than by comparing every path with every other one: a
     select-all in a large folder made that O(n²), and each comparison built a
     string to do it. A path is carried by another exactly when one of the
     folders above it is in the set, and there are as many of those as the path
     is deep. */
  const all = new Set(paths)
  return [...all].filter((p) => {
    let at = p.lastIndexOf('/')
    while (at > 0) {
      if (all.has(p.slice(0, at))) return false
...
/**
 * Select every visible row between two.
 *
 * Shared by the mouse and the keyboard on purpose: shift-click draws the same
 * range shift-↓ extends, and one of the two getting it right while the other
 * did not is how a tree ends up with two ideas of what is selected. Answers
 * false when either end has gone off screen — a folder shut under the anchor,
 * a file deleted — which is the caller's cue to fall back to the one row it
 * was actually given.
 */
function pickRange (anchor, path) {
  ...
  /* The far end of a range becomes current without disturbing the anchor, so a
     second shift — click or key — re-extends from the same place rather than
     from wherever the last one landed. */
  state.pickAnchor = anchor
  markPicked()
  return true
}

/** One row alone, and the place a later range measures from. */
function pickOnly (path) {
...
/**
 * Opening from the tree, the way a file manager does it: the click adds a tab
 * rather than taking the document being read out from under it. A file already
 * on the strip is a tab to go back to, not a second copy of itself — which is
 * also what keeps a walk through a folder from stacking the same note twice.
 */
async function openFromTree (path) {
  ...
  return openNote(path, { newTab: true })
}
...
function clickRow (node, event) {
  ...
  if (event.shiftKey && anchor && pickRange(anchor, path)) {
    /* markPicked runs again after the open, because opening a note clears the
       status line the count is written into. */
    if (node.type === 'file') openFromTree(path).then(markPicked)
    return
  }
...
/* No double-click rename in the tree. A folder opens and closes on a single
   click, so the second click of an impatient double-click on a folder landed in
   a rename nobody asked for. Renaming is the context menu's Rename… and ↵ on
   the selected row, both of which say what they are before they start. */
...
async function importFrom (event, destDir) {
  ...
  try {
    ...
    toast(reason(err, 'Those files could not be brought in.'))
    return
  }
...
/** Whether `p` can be moved into `dir`: not into itself or its own subtree,
 *  and not into the folder it is already sitting in. */
...
function moveDestinations (paths) {
  const wanted = paths.filter(Boolean)
  const holds = (dir) => wanted.every((p) => canMove(p, dir))
...
function canDropInto (destDir) {
  return (state.dragging || []).some((p) => canMove(p, destDir))
}
...
async function moveInto (destDir, paths = state.dragging || []) {
  ...
  for (const path of paths) {
    ...
    try {
      ...
      relinked += links
      /* A collision at the destination is settled by suffixing — main lands the
         file on `Name 2` rather than refusing the drop — which is the right
         outcome and the wrong thing to do silently: the reader watching
         "Report" arrive as "Report 2" deserves to be told it was a collision
         and not a glitch. */
      const was = path.split('/').pop()
      const now = next.split('/').pop()
      if (was !== now) toast(`“${was}” already existed there, so this one is “${now}”.`)
    } catch (err) {
      toast(reason(err, `“${path}” could not be moved.`))
    }
  }
...
/** The node at a vault path, walked down from the top of the tree. */
function treeNodeAt (path) {
  let nodes = state.tree
  let acc = ''
  for (const part of path.split('/')) {
    acc = acc ? `${acc}/${part}` : part
    const hit = nodes?.find((n) => n.path === acc)
    if (!hit) return null
    if (acc === path) return hit
    nodes = hit.children
  }
  return null
}
...
/**
 * Draw one closed folder open, in place.
 *
 * Its container is already in the DOM — `buildLevel` draws the pair and leaves
 * the container empty for a folder that is shut — so opening one is filling
 * that container rather than rebuilding the tree around it. Rebuilt rather than
 * revealed, because a container belonging to a folder that has been shut is
 * skipped by `patchTree` and so may be showing a vault that has since moved on.
 *
 * @returns {boolean} whether it could be done here; false asks for a rebuild.
 */
function unfoldRow (path) {
  const row = el.tree.querySelector(`.row[data-path="${cssEscape(path)}"]`)
  const kids = el.tree.querySelector(`.children[data-for="${cssEscape(path)}"]`)
  const node = treeNodeAt(path)
  if (!row || !kids || !node) return false
  row.classList.add('is-open')
  row.setAttribute('aria-expanded', 'true')
  kids.classList.add('is-open')
  kids.replaceChildren(buildLevel(node.children || [], path.split('/').length))
  return true
}
...
function revealInTree (path) {
  ...
  for (const part of parts) {
    acc = acc ? `${acc}/${part}` : part
    if (state.expanded.has(acc)) continue
    state.expanded.add(acc)
    /* A folder that was closed has to be drawn open. That used to mean
       `renderTree()` — every row in the vault thrown away and built again, with
       its icons and its half-dozen listeners — to show the contents of one
       folder. It is also the common case rather than the rare one: revealing
       follows almost every note opened from a search or a backlink, and those
       are exactly the notes that live somewhere the reader had not unfolded.
       Top-down, so each folder's container exists by the time it is reached. */
    if (!unfoldRow(acc)) rebuild = true
  }
  if (rebuild) renderTree()
  else markActiveRow()
...
function enterDoc (path, { history, newTab }) {
  ...
  /* Only files become lasting tabs. A temporary tab (logs.md) is never reused
     for a file — the file opens beside it — and with no file placeholder the
     empty strip simply gains its first tab. The one exception is the singleton
     "New tab": an open of any file while it is showing fills it, so a tree
     click from it does not strand an empty tab beside the file. */
  const reuse = state.tabs.length && (isNewTab(activeTab()) || (!newTab && !activeTab()?.memory))
  if (!reuse) {
    ...
    const tab = { path: null, history: [], historyAt: -1, base: null, pinned: false }
    const at = state.tabs.length ? state.tabIndex + 1 : 0
...
/* The mtime and size the open note's text was read at — the version of the
   file the buffer is an edit OF. Each autosave hands it back as `expect`, and
   main refuses the write if the file on disk is no longer that version: the
   watcher already catches an outside edit, but only after its 180 ms debounce,
   and a sync client pulling the other side down inside that window used to be
   silently overwritten. Refreshed by every path that brings buffer and disk
   back into agreement — open, save, reload, merge. */
let noteStamp = { path: null, stamp: null }

const holdNoteStamp = (path, stamp) => {
  noteStamp = stamp ? { path, stamp } : { path: null, stamp: null }
}

async function openText (path, { focus = true, history = true, place = null, newTab = false, chat = true, bookmark = place == null } = {}) {
  ...
  try {
    ...
    text = got.text
    holdNoteStamp(path, got.stamp)
  } catch (err) {
    /* Refused for its size is a different fact from unreadable, and "refreshing
       the vault" would be a strange answer to it. */
    toast(reason(err, 'That note could not be opened. Refreshing the vault.'))
    if (!/too large/i.test(String(err?.message || ''))) await loadTree()
    return false
  }
  ...
  if (isLanguageTablePath(path)) {
    const [{ normalizeLanguageTable }] = await Promise.all([
      import('./language-table.js'),
      loadFeatureStyles('language')
    ])
    const normalized = normalizeLanguageTable(text)
    if (normalized !== text) {
      /* Tidying the table is a courtesy, not the reason the file was opened.
         A write that fails here — a read-only vault, a file locked by a sync
         client — used to reject `openText` in the middle of opening, after the
         buffer had been set and before `enterDoc` ever ran: a tab that showed
         nothing and said nothing. The un-normalised text opens perfectly well. */
      try {
        const wrote = await api.file.write(path, normalized)
        text = normalized
        // The tidy moved the file on from the version just read; the buffer is
        // now an edit of the tidied version, and the autosave must say so.
        holdNoteStamp(path, wrote?.stamp)
      } catch (err) {
        toast(reason(err, 'This table could not be tidied, so it is shown as it is on disk.'))
      }
    }
  }
  ...
  /* A bookmarked note opens at its bookmark, centred. Only on a fresh open —
     coming back to a tab returns to where it was left, which is what the tab's
     own place is for; the bookmark is where the *note* is picked up again. It
     takes the place's turn entirely: two scrolls aimed at different lines in
     the same frame only ever leave the page at one of them by chance. */
  const marked = bookmark && !code && !tex && bookmarkLineOf(text) > 0
  if (place && !marked) {
...
async function openViewed (path, { focus = true, history = true, place = null, newTab = false, chat = true }, viewer) {
  ...
  try {
    ...
    toast(reason(err, viewer.failed))
    ...
    if (at !== -1) {
      ...
      state.tabIndex = state.tabs.length ? Math.min(indexBefore, state.tabs.length - 1) : -1
    }
    const tab = activeTab()
    if (tab && previous?.path) tab.path = previous.path
 ...
 *
 * A viewed kind: there is no text editor behind it, and what is typed into the
 * page is saved by the docx view itself, which splices the paragraphs back into
 * the file it read them from (src/docx.js). A file that will not unzip, or that
 * has no `word/document.xml` in it, throws — and `openViewed` hands the tab
 * back rather than leaving an empty page claiming to be a document.
 */
...
/* One consent decision covers fenced code, source files and notebook kernels.
   Main enforces it too; this prompt is only the human-facing path to grant it. */
async function mayRunCode () {
  if (await api.run.trusted().catch(() => false)) return true
  const name = state.vault?.name || 'this vault'
  const yes = await ask({
    title: `Allow code to run in “${name}”?`,
  ...
  if (!yes) return false
  const trusted = await api.run.trust().catch(() => false)
  if (!trusted) toast('This vault could not be trusted.')
  return trusted
}
resolveRunPermission(mayRunCode)
...
function applyPanes () {
  ...
  const flashcardOpen = viewingFlashcardBank()
  ...
  const tabular = viewingLanguageTable() || dataOpen || notebookOpen || docxOpen || viewingHtml() || flashcardOpen
  ...
  el.reading.hidden = !text || flashcardOpen || sourceOnly || htmlOpen || state.view !== 'read'
  el.editorHost.hidden = !text || flashcardOpen || (!sourceOnly && state.view === 'read')
  el.flashcardBank.hidden = !flashcardOpen
  if (flashcardOpen) paintFlashcardBank()
  ...
  /* The toolbar's Run: only over a source file in a language the run
     ...
     empties the dock; the same file merely changing views keeps it. */
  const runnableFile = codeOpen && !lockedHere() &&
    isRunnable(codeToken(state.current?.path || ''))
  el.docRun.hidden = !runnableFile
  ...
  updateViewControl()
  const bankCards = viewingFlashcardBank() ? parseFlashcards(noteText()) : []
  const studyable = viewingLanguageTable() || bankCards.length > 0
  el.studyStart.hidden = !studyable
  el.studyStart.title = viewingFlashcardBank() ? 'Study this flashcard bank' : 'Study this language table'
  /* The empty bank itself owns its one primary action. The toolbar shortcut
     arrives once there is a bank to extend, so zero cards no longer produces
     three equally loud Add buttons. */
  el.flashcardBankAdd.hidden = !viewingFlashcardBank() || readOnlyHere() || !bankCards.length
  paintKeyboard()
  ...
  api.window.menuContext(notebookOpen ? 'notebook' : '')
  paintZoomBadge()
...
/* Only files (and the temporary logs.md tab) persist: closing the last file
   tab leaves the strip empty. A single transient "New tab" may stand on the
   strip beside them — see `newTab` — and it is never written down. */

/** A transient empty tab: no file, no temporary document. Only one may exist. */
const isNewTab = (tab) => !!tab && !tab.path && !tab.memory

/** Where the singleton "New tab" stands, or -1 when there is none. */
const findNewTab = () => state.tabs.findIndex(isNewTab)
...
function renderTabs () {
  ...
  state.tabs.forEach((tab, i) => {
    ...
    /* Which document is showing, said out loud. The strip looked like a row of
       ...
       you are looking at. `aria-current` is also what `returnFocus` above
       reaches for when a collapsing sidebar has to put focus somewhere: it was
       querying for an attribute nothing ever set, so focus went to the body
       and Tab restarted from the top of the document. */
    button.setAttribute('role', 'tab')
    button.setAttribute('aria-selected', String(active))
    if (active) button.setAttribute('aria-current', 'true')
    /* Roving: one stop for the whole strip, arrows to move within it, the way
       the sidebar's own tablist already works. */
    ...
    if (tab.pinned) {
      ...
      /* A span rather than a button: a button inside a button is invalid, and
         the browser's own hit-testing stops working when you nest them. Saying
         `role="button"` without a tabindex, though, announced a control that
         could not then be reached or pressed — so it carries both, and Enter
         and Space do what a real button's would. */
  ...
  /* Every button in the strip is about to be destroyed and rebuilt. If the
     reader was standing on one — they had just pressed ⌘W, or the × — focus
     would land on the body and Tab would restart from the top of the document,
...
/* Arrows walk the strip, Home and End reach its ends — the roving tabindex set
   in renderTabs is only half of the pattern, and this is the half that moves.
   Bound once here rather than per button, because the buttons are replaced
...
el.tabs.addEventListener('keydown', (e) => {
  ...
  const to = e.key === 'Home'
    ...
    : e.key === 'End'
      ? tabs.length - 1
      : (at + STEP[e.key] + tabs.length) % tabs.length
  /* Moving focus only. Selecting as you arrow would read every document you
     passed over off disk, which on a strip of eight is seven notes nobody
     asked for; Enter or Space on the tab is what opens it. */
  tabs[to].tabIndex = 0
  tabs[at].tabIndex = -1
  tabs[to].focus()
})
...
function showTabContextMenu (event, i) {
  ...
  if (tab.path) {
    ...
    /* "Show in explorer" sat directly above "Reveal in Finder", and the two
       read as the same thing said twice — one means Tulip's own Files pane and
       the other means the desktop's file manager. Named after the pane it
       actually opens. */
    items.push({ label: 'Show in Files pane', run: () => showInFileExplorer(tab.path) })
    /* Moving and trashing are what the tree's row offers for the same
       document. This menu's own comment has always said a tab "offers what a
       row in the Files pane offers for one", and until now it did not: the
       two were reachable only from a tree the reader may have scrolled away
       from or closed altogether. Recovery and opening beside went the other
       way — out of both menus, into the palette, where an action reached for
       once in a while belongs. */
...
async function closeTabs (doomed, keeper) {
  ...
  state.tabIndex = to === -1 ? (state.tabs.length ? Math.min(state.tabIndex, state.tabs.length - 1) : -1) : to
  if (already) { renderTabs(); rememberTabs(); return }

  const next = activeTab()
  if (!next) { await showEmpty(); return }
  if (next.memory) { await showMemoryDocument(next); return }
  if (isNewTab(next)) { closeCurrentNote(); copilot.setNote(''); rememberTabs(); return }
...
function rememberTabs () {
  ...
  markPlace()
  /* The transient tabs are never written down: logs.md is rebuilt on demand,
     and the singleton "New tab" is a place to start from rather than a strip
     to come back to. */
  const savedTabs = state.tabs.filter((tab) => !tab.memory && tab.path)
  sessionOnly({
...
async function selectTab (i) {
  ...
  if (tab.memory) { await showMemoryDocument(tab); return true }
  /* The singleton empty tab shows the same empty pane as having no tabs at
     all. Saved and marked above, like every other selection; torn down below,
     the way opening a file tears its viewer down on the way in. */
  if (isNewTab(tab)) { closeCurrentNote(); copilot.setNote(''); rememberTabs(); return true }
...
/**
 * The singleton "New tab", on ⌘T.
 *
 * A second press while one stands finds it instead of stacking another: the
 * strip never holds two tabs with nothing in them. Opening any file while it
 * is showing fills it — see `enterDoc` — so it is a place to start from
 * rather than a tab to keep.
 */
async function newTab () {
  const at = findNewTab()
  if (at >= 0) {
    if (at !== state.tabIndex) await selectTab(at)
    return true
  }
  if (state.dirty && !await saveNow()) return false
  markPlace()
  const tab = { path: null, history: [], historyAt: -1, base: null, pinned: false }
  const insert = state.tabs.length ? state.tabIndex + 1 : 0
  state.tabs.splice(insert, 0, tab)
  state.tabIndex = insert
  closeCurrentNote()
  copilot.setNote('')
  rememberTabs()
  return true
...
/* The tabs closed this session, newest last, so ⌘⇧T can walk back through
   them. Tabs closed because their file went away are left out to keep a
...
async function closeTab (i, { record = true } = {}) {
  const tab = state.tabs[i]
  if (!tab) return
  if (i === state.tabIndex && state.dirty && !await saveNow()) return false
  ...
  // Closing the last tab leaves the strip empty and the pane in its empty
  // state. Only files become tabs, so there is no blank placeholder.
  ...
  if (i < state.tabIndex) { state.tabIndex--; renderTabs(); rememberTabs(); return true }
  if (i > state.tabIndex) { renderTabs(); rememberTabs(); return true }
  ...
  if (next.memory) { await showMemoryDocument(next); return true }
  if (isNewTab(next)) { closeCurrentNote(); copilot.setNote(''); rememberTabs(); return true }
  await openNote(next.path, { history: false, place: tabPlace(next) })
  revealInTree(next.path)
  return true
}
...
/** Put the pane back to its empty state when no tab is showing. */
async function showEmpty () {
  /* Edits are saved, not dropped, before the note is abandoned — the same
     promise selectTab and closeTab make. */
  if (state.dirty && !await saveNow()) return false
  closeCurrentNote()
  copilot.setNote('')
  rememberTabs()
...
async function goHistory (delta) {
  ...
  /* A watcher can remove a document between the moment this trail was drawn
     ...
     is recovery chrome and a stale stop should be skipped without an error
     toast. */
  if (!state.files.some((file) => file.path === entry.path)) {
...
async function reloadCurrent () {
  ...
  /* A table rewritten underneath is reread in place, scrolled where it was.
     (Unsaved cells of the reader's own stand: the `dirty` check above. ) */
  if (viewingData()) {
  ...
  let text
  try {
    const got = await api.file.readStamped(state.current.path)
    text = got.text
    // Buffer and disk are about to agree on this version; the next autosave
    // measures against it.
    holdNoteStamp(state.current.path, got.stamp)
  } catch { return }
...
function rememberAgentBefore (relPath, needle = '', tool = 'Edit', turn = '') {
  ...
  /* The note on screen answers from its own buffer, unsaved edits included.
     Anything else must not be read off the disk here: a provider announces a
     Write after it has happened, so by this moment the file can already be the
     copilot's version — for a created file, before == after, and the review
     this capture exists for never appears. The turn's baseline is the copy
     taken before the turn could write anything, and it is the same copy the
     turn-end review card is diffed against. A turn without one (`known` false)
     falls back to the disk, which is then all there is. */
  const before = onScreen
    ? Promise.resolve(currentText)
    : api.ai.baseline(turn, relPath)
        .then((base) => (base?.known ? base.text : api.file.read(relPath)))
...
async function absorbAgentEdit (relPath) {
  ...
  if (relPath !== state.current?.path) {
    ...
    return isEditableTextPath(relPath)
      ? agentEditSummary(before, noteText())
      : null
  }
...
/** Accepting, like a successful restore, dismisses the editor-side review. */
async function acceptAgentChanges (operation) {
  ...
  clearVisibleAgentDiff(cleared)
}

function clearVisibleAgentDiff (paths) {
  if (paths.has(state.current?.path)) editor?.clearAgentDiff()
}

function agentRestorePaths (operation, path = null) {
  return new Set(path == null
    ? operation.changes.map((change) => change.path)
    : [path])
}

/** Reject is visually final as soon as it is confirmed, before file I/O. */
function beginAgentRestore (operation, path = null) {
  clearVisibleAgentDiff(agentRestorePaths(operation, path))
}

/** A failed restore keeps its pending review, so put its diff back on screen. */
async function redrawAgentRestore (operation, path = null) {
  const current = state.current?.path
  if (current && agentRestorePaths(operation, path).has(current)) {
    await showAgentReview(current, operation.id)
  }
}

/** A successful restore settles the review for every file it put back. */
function settleAgentRestore (operation, path = null) {
  const restored = agentRestorePaths(operation, path)
  for (const restoredPath of restored) pendingAgentDiffs.delete(restoredPath)
}
...
const noteHistory = mountHistory({
  ...
  restoreStarted: beginAgentRestore,
  restoreFailed: redrawAgentRestore,
  afterRestore: settleAgentRestore,
  onError: toast
})
...
async function handleDiskConflict (path) {
  ...
  try {
    const got = await api.file.readStamped(path)
    disk = got.text
    /* The merge is against THIS version of the file; the save that follows it
       writes over this version and must say so, or it would be refused as
       stale against the very disk text it just merged in. */
    holdNoteStamp(path, got.stamp)
  } catch {
    mergeOpen = false
    return false
  ...
  /* The file on disk is still the version the buffer started from: the event
     that brought us here was a touch, not a rewrite — a sync client restamping
     the file, a watcher echo outside the self-write window. There is nothing
     to merge, and a merge from here would only ever pair the buffer's own
     edits against themselves. The autosave this handler stopped is put back. */
  if (disk === base) {
    mergeOpen = false
    if (state.dirty) await saveNow()
    return true
 ...
 * and unchanged.
 */
/* What the open note looked like on disk the last time this checked.

   The check below runs on every window focus, and it used to answer the
   question by reading the whole file back over IPC and comparing strings —
   which for a note in the megabytes is a copy across the process boundary every
   time the reader alt-tabs back in. A stat answers the same question for the
   size of a stat, and the read only has to happen when the answer is yes. */
let diskStamp = { path: null, modified: 0, size: -1 }

async function recheckOpenNote () {
  const path = state.current?.path
  if (!path || viewingPdf() || !isEditableTextPath(path)) return

  /* Unchanged mtime AND unchanged size. Either alone can be fooled — a file
     restored from a backup keeps its size, a filesystem with coarse timestamps
     keeps its mtime through a fast rewrite — and the cost of being wrong here
     is a stale buffer overwriting somebody else's edit at the next autosave.
     A failed stat says nothing, so it falls through to the read. */
  const seen = await api.file.info(path).catch(() => null)
  if (seen?.ok && diskStamp.path === path &&
  ...
  try {
    const got = await api.file.readStamped(path)
    disk = got.text
    stampOnDisk = got.stamp
  } catch { return }
  if (seen?.ok) diskStamp = { path, modified: seen.modified, size: seen.size }
  // The note may have been closed or switched while the read was in flight.
  if (state.current?.path !== path) return
  /* Whatever happens next, this is now the version of the file on disk — a
     touched mtime with identical text included — and the autosave's staleness
     check should measure against it rather than tripping over a change that
     changed nothing. */
  ...
  if (disk === noteText()) { holdNoteStamp(path, stampOnDisk); return }
...
const copilotDeps = {
  ...
  onOpen: async (path, line = null, operationId = null) => {
    ...
    if (line != null && state.current?.path === path) {
      if (reading()) setView('edit')
      goToLine(Math.max(1, Math.min(line, noteLines())))
    }
  ...
  onPermission: ({ providerLabel, grant, model }) => ask({
    ...
    detail: `${providerLabel} will use ${model}. It may ${grant || 'read, edit and create files inside the vault'} for the rest of this chat.`,
    go: 'Allow for this chat'
  }),
  /* A reply put into the note at the caret, replacing whatever is selected.
     Only into a text document that is open for editing: there is nowhere in
     a PDF or a grid to put a paragraph, and a reading view has no caret. The
     answer says whether it went, so the panel can say why not. */
  onInsert: (text) => {
    if (!editor || !viewingText() || reading()) return false
...
function loadCopilot () {
  ...
  copilotArriving ||= Promise.all([
    import('./copilot.js'),
    loadFeatureStyles('copilot')
  ]).then(async ([{ mountCopilot }]) => {
...
const copilot = {
  ...
  ask: (text, options) => loadCopilot().then((c) => c.ask(text, options)),
...
onAskToFix((prompt) => {
  copilot.open()
  copilot.ask(prompt, { contextMode: CONTEXT_MODES.CODE_TASK })
})
...
codeAiPop.addEventListener('submit', (event) => {
  ...
  copilot.ask(
    ...
    `Change nothing else.\n\nRequest: ${request}\n\n` +
    `${fence}${lang}\n${code}\n${fence}`,
    { contextMode: CONTEXT_MODES.CODE_TASK }
  )
})
...
async function collectCopilotContext (options = {}) {
  ...
  const sourceFile = code || viewingTex()
  const skipExcerpt = options.mode === CONTEXT_MODES.CODE_TASK && code

  /* The viewer is imported with the first PDF of the session, and the tab is
     claimed before it lands. A question asked in that gap is about a document
     there is nothing to quote from yet — the same empty answer the other
     viewers below give before they have mounted. */
  if (viewingPdf()) {
    const quote = pdf?.quote()
    const selection = boundedText(quote?.text, budget.selection)
    return done({
      note: state.current.path,
      kind: 'pdf',
      page: quote?.page || pdf?.page() || 1,
      pages: pdf?.pages() || 0,
  ...
  /* A website is described by the page, not by the file. The file says one
     ...
     links deep from the address the file names.

     And the page's own words, which used not to travel at all — so "summarise
     this" was impossible over the one document kind where it is the most
     natural thing to ask. What crosses is a string the guest was asked for by
  ...
  /* A Word document is a zip, so send the selection and a focused paragraph
     window extracted by the viewer rather than pretending it is plain text. */
  if (viewingDocx()) {
    const view = docxInstance?.context?.() || { text: '', selection: '', focus: 0 }
    const selection = boundedText(view.selection, budget.selection)
    const preview = boundedText(view.text, budget.structured, view.focus)
    return done({
      note: state.current.path,
      kind: 'docx',
      title: view.title || docxInstance?.title() || '',
      words: view.words || docxInstance?.words() || 0,
      selection: selection.text,
      text: preview.text,
      paragraphs: view.paragraphs || 0,
      at: view.at || 0,
      truncated: selection.truncated || preview.truncated
    })
  }
  ...
  if (viewingWhiteboard()) {
    ...
    const selection = boundedText(board.selection, budget.selection)
    const text = boundedText(board.text, budget.structured)
    return done({
      note: state.current.path,
      kind: 'whiteboard',
      selection: selection.text,
      ...
      truncated: selection.truncated || text.truncated
    })
  }
  ...
  if (viewingData()) {
    ...
    const preview = boundedText(table.text, budget.structured, table.focus)
    return done({
      note: state.current.path,
      kind: 'data',
      selection: '',
  ...
  if (viewingNotebook()) {
    const book = notebookInstance?.context() || { text: '', cells: 0, language: '' }
    const preview = boundedText(book.text, budget.structured, book.focus)
    return done({
      note: state.current.path,
      kind: 'notebook',
      selection: '',
  ...
  const kind = textContextKind({
    tex: viewingTex(),
    code: Boolean(code),
    language: viewingLanguageTable(),
    flashcards: viewingFlashcardBank(),
    codeLanguage: languageLabel(codeToken(state.current?.path)) || 'source'
  })

  /* What travels of the document itself — the same five fields either way, and
     the reason `skipExcerpt` is answered here rather than only in the prompt: a
     focused code task is never shown the open file, so building the window and
     sending it across the bridge for `promptFor` to drop is a slice and up to
     twelve thousand characters of IPC spent on nothing. The size still goes,
     because "how big is the file you are not being shown" is worth knowing. */
  const body = (text, head) => {
    const excerpt = skipExcerpt
      ? { text: '', cut: false }
      : noteExcerpt(text, head, { code: sourceFile, maxChars: budget.document })
    return {
  ...
  const boundedSelection = boundedText(selection, budget.selection)
  return done({
    note: state.current?.path || '',
    kind,
    selection: boundedSelection.text,
    truncated: boundedSelection.truncated,
...
async function goToCitation (path, page) {
  ...
  /* Opening it above leaves the viewer up, but arriving with it already on
     screen does not: the first PDF of a session claims its tab while the module
     is still being imported, and a citation followed into that gap has no pages
     to count yet. The document is a moment from being ready, and clicking the
...
/* Study, speech, the on-screen keyboard and its alphabet catalogue are a
   language route, not shell furniture. Opening an ordinary note no longer
   compiles or evaluates them; the first language note or Study press builds
   the set once. */
function ensureLanguageRuntime () {
  if (languageStudy && keyboard) return Promise.resolve({ languageStudy, keyboard })
  languageLoading ||= Promise.all([
    import('./language-table.js'),
    import('./speech.js'),
    import('./keyboard.js'),
    import('../electron/alphabets.json'),
    loadFeatureStyles('language')
  ]).then(([{ mountLanguageStudy }, { makeSpeech }, { mountKeyboard }, alphabets]) => {
    languageAlphabets = alphabets.default
    speech = makeSpeech()
    languageStudy = mountLanguageStudy({
        el: {
          root: $('study'),
          title: $('study-title'),
          card: $('study-card'),
    ...
    return { languageStudy, keyboard }
  }).finally(() => { languageLoading = null })
...
/* A .fc session is deliberately local to the open bank. It shuffles once at
   the start (and once when its tag changes), then walks that fixed queue so no
   card repeats until the whole selected area has been seen. */
 ...
 *  replaces when a card is edited from the bank. */
function bankEditTargetFor (card, source) {
  const offsets = [0]
  for (const ending of String(source).matchAll(/\r?\n/g)) offsets.push(ending.index + ending[0].length)
  const from = offsets[card.start] ?? 0
  const to = offsets[card.end] ?? source.length
  return { card, from, to, source: source.slice(from, to) }
}

function paintFlashcardBank () {
  if (!viewingFlashcardBank()) return
  const source = noteText()
  const cards = parseFlashcards(source)
  const emptyBank = !cards.length
  el.flashcardBank.classList.toggle('is-empty', emptyBank)
  el.flashcardBankCount.textContent = `${cards.length} ${cards.length === 1 ? 'card' : 'cards'}`
  el.flashcardBankAddMain.hidden = emptyBank || readOnlyHere()
  el.flashcardBankTopics.hidden = emptyBank
  el.flashcardBankTopicList.replaceChildren()
  el.flashcardBankList.replaceChildren()

  if (cards.length) {
    const studyAll = node('button', 'flashcard-bank-topic', `All cards · ${cards.length}`)
    studyAll.type = 'button'
    studyAll.addEventListener('click', () => openFlashcardStudy())
    el.flashcardBankTopicList.append(studyAll)
    for (const tag of flashcardTags(cards)) {
      const count = cards.filter((card) => card.tags.some((item) =>
        item.toLocaleLowerCase() === tag.toLocaleLowerCase())).length
      const button = node('button', 'flashcard-bank-topic', `${tag} · ${count}`)
      button.type = 'button'
      button.title = `Study ${tag}`
      button.addEventListener('click', () => openFlashcardStudy(tag))
      el.flashcardBankTopicList.append(button)
    }
  } else {
    el.flashcardBankTopicList.append(node('p', 'flashcard-bank-empty',
      'Tags will appear here after you add your first flashcard.'))
  }

  cards.forEach((card, index) => {
    const row = node('article', 'flashcard-bank-card')
    const number = node('span', 'flashcard-bank-number', String(index + 1).padStart(2, '0'))
    const content = node('div', 'flashcard-bank-card-content')
    content.append(node('h3', '', card.question))
    content.append(node('p', 'flashcard-bank-answer', `Correct answer: ${card.options[card.correct]}`))
    if (card.tags.length) {
      const tags = node('div', 'flashcard-bank-card-tags')
      card.tags.forEach((tag) => tags.append(node('span', '', tag)))
      content.append(tags)
    }
    const edit = node('button', 'flashcard-bank-edit', 'Edit')
    edit.type = 'button'
    edit.addEventListener('click', () => openFlashcardComposer(bankEditTargetFor(card, source)))
    row.append(number, content, edit)
    el.flashcardBankList.append(row)
  })

  if (!cards.length) {
    const empty = node('div', 'flashcard-bank-empty-state')
    empty.append(
      node('h3', '', 'Build your flashcard bank'),
      node('p', '', 'Add questions, answers and tags here. Tulip handles the file for you.')
    )
    const add = node('button', 'ask-btn', 'Add your first flashcard')
    add.type = 'button'
    add.hidden = readOnlyHere()
    add.addEventListener('click', () => openFlashcardComposer())
    empty.append(add)
    el.flashcardBankList.append(empty)
  }
}

function closeFlashcardStudy () {
  if (el.fcStudy.hidden) return
  el.fcStudy.hidden = true
  const study = fcStudy
  fcStudy = null
  /* Answers owed to the vault go before the overlay is gone — the same barrier
     the language session closes behind. */
  study?.sink?.settle().catch((error) => reportFailure('saving review', error))
  el.studyStart.focus()
}

function paintFlashcardStudyTags () {
  const study = fcStudy
  if (!study) return
  el.fcStudyTags.replaceChildren()
  for (const tag of ['', ...flashcardTags(study.cards)]) {
    const button = node('button', 'fc-study-tag', tag || 'All cards')
    button.type = 'button'
    button.setAttribute('role', 'radio')
    button.setAttribute('aria-checked', String(tag === study.tag))
    button.addEventListener('click', () => resetFlashcardStudy(tag))
    el.fcStudyTags.append(button)
  }
}

function answerFlashcardStudy (selected) {
  const study = fcStudy
  if (!study || study.answered) return
  const card = study.queue[study.index]
  if (!card) return
  study.answered = true
  const right = selected === card.correct
  if (right) study.correct++
  else study.wrong++

  /* Multiple choice is a recognise card: a right answer is GOOD, a wrong one
     AGAIN — and an AGAIN comes back inside the session, because a card just
     failed is one not learned yet. The store's id is the card's own, so the
     same schedule follows the question wherever the bank's file moves. */
  const now = Date.now()
  const grade = right ? GOOD : AGAIN
  const next = gradeCard(study.states[card.id], grade, now, state.cfg.studyRetention)
  study.states[card.id] = next
  study.sink?.offer({ id: card.id, at: now, grade, state: next })
  study.done++
  if (!right) study.queue.push(card)
  const buttons = [...el.fcStudyOptions.querySelectorAll('button')]
  buttons.forEach((button, index) => {
    button.disabled = true
    button.classList.add(index === card.correct ? 'is-correct' : 'is-wrong')
    if (index === selected) button.classList.add('is-selected')
  })
  el.fcStudyFeedback.dataset.result = right ? 'correct' : 'wrong'
  el.fcStudyFeedback.textContent = right
    ? 'Correct.'
    : `The correct answer is “${card.options[card.correct]}”.`
  el.fcStudyFeedback.hidden = false
  el.fcStudyExplanation.textContent = card.explanation
  el.fcStudyExplanation.hidden = !card.explanation
  el.fcStudyNext.textContent = study.index + 1 >= study.queue.length ? 'Finish' : 'Next'
  el.fcStudyNext.hidden = false
  el.fcStudyNext.focus()
}

function paintFlashcardStudy () {
  const study = fcStudy
  if (!study) return
  const card = study.queue[study.index]
  if (!card) {
    el.fcStudyCard.hidden = true
    el.fcStudyFinished.hidden = false
    /* A session that answered nothing was one with nothing due — say so, rather
       than reporting a score of zero as though cards had been studied. */
    el.fcStudyProgress.textContent = study.done
      ? `${study.done} reviewed · ${study.tag || 'All areas'}`
      : 'Nothing due — every card is scheduled for later'
    el.fcStudyScore.textContent = `${study.correct} correct · ${study.wrong} missed`
    el.fcStudyScore.hidden = !study.done
    el.fcStudyAgain.hidden = !study.done
    if (study.done) el.fcStudyAgain.focus()
    else el.fcStudyClose.focus()
    return
  }

  study.answered = false
  el.fcStudyFinished.hidden = true
  el.fcStudyCard.hidden = false
  el.fcStudyProgress.textContent = `${study.index + 1} of ${study.queue.length}` +
    (study.tag ? ` · ${study.tag}` : '')
  el.fcStudyQuestion.textContent = card.question
  if (card.image) {
    el.fcStudyImage.src = assetUrl(resolveHere(card.image) || card.image)
    el.fcStudyImage.alt = `Image for ${card.question}`
    el.fcStudyImage.hidden = false
  } else {
    el.fcStudyImage.hidden = true
    el.fcStudyImage.removeAttribute('src')
    el.fcStudyImage.alt = ''
  }
  el.fcStudyOptions.replaceChildren()
  card.options.forEach((option, index) => {
    const button = node('button', `fc-study-option is-tone-${index % 4}`, option)
    button.type = 'button'
    button.addEventListener('click', () => answerFlashcardStudy(index))
    el.fcStudyOptions.append(button)
  })
  el.fcStudyFeedback.hidden = true
  el.fcStudyFeedback.textContent = ''
  el.fcStudyExplanation.hidden = true
  el.fcStudyExplanation.textContent = ''
  el.fcStudyNext.hidden = true
  el.fcStudyOptions.querySelector('button')?.focus()
}

function resetFlashcardStudy (tag = fcStudy?.tag || '') {
  const study = fcStudy
  if (!study) return
  study.tag = tag
  const wanted = tag.replace(/^#/, '').toLocaleLowerCase()
  const pool = wanted
    ? study.cards.filter((card) => (card.tags || []).some((item) => item.toLocaleLowerCase() === wanted))
    : study.cards
  study.queue = buildQueue(pool, study.states, Date.now(), { newPerDay: state.cfg.studyNewPerDay })
  study.index = 0
  study.done = 0
  study.correct = 0
  study.wrong = 0
  paintFlashcardStudyTags()
  paintFlashcardStudy()
}

/* The deck a session is built from: this bank's cards, or every .fc bank in
   the vault — the same split the language study's own/vault-wide modes make. */
async function dealFlashcardStudy (cards, title, tag = '') {
  if (!cards.length) { toast('Add at least one complete flashcard before studying.'); return }
  let states = {}
  try {
    states = await api.review.all()
  } catch (error) {
    reportFailure('loading review state', error)
  }
  fcStudy = {
    cards, tag, queue: [], index: 0, done: 0,
    correct: 0, wrong: 0, answered: false, states,
    sink: makeReviewSink({ record: api.review.record, unrecord: api.review.unrecord })
  }
  el.fcStudyTitle.textContent = title
  el.fcStudy.hidden = false
  resetFlashcardStudy(tag)
}

function openFlashcardStudy (tag = '') {
  if (!viewingFlashcardBank()) return
  const cards = flashcardCards(noteText(), state.current.path)
  dealFlashcardStudy(cards, `Study ${state.current.name}`, tag)
    .catch((error) => reportFailure('flashcard study', error))
}

/* Every bank at once: the vault-wide counterpart of the study button, reached
   from the command palette wherever the user happens to be. */
function openFlashcardStudyAll () {
  api.flashcards.banks()
    .then((banks) => dealFlashcardStudy(
      banks.flatMap((bank) => flashcardCards(bank.text, bank.path)),
      'Study all due flashcards'
    ))
    .catch((error) => reportFailure('flashcard study', error))
}

/* The button beside the tabs serves whichever dedicated study document is on
   screen: language tables run their typed-answer session; .fc banks run the
   scheduled multiple-choice queue above. Both keep the same calendar. */
el.studyStart.addEventListener('click', () => {
  if (viewingFlashcardBank()) { openFlashcardStudy(); return }
  ensureLanguageRuntime()
    .then(({ languageStudy }) => languageStudy.open())
    .catch((error) => reportFailure('language study', error))
})
el.flashcardBankAdd.addEventListener('click', () => openFlashcardComposer())
el.flashcardBankAddMain.addEventListener('click', () => openFlashcardComposer())
el.fcStudyClose.addEventListener('click', closeFlashcardStudy)
el.fcStudyNext.addEventListener('click', () => {
  if (!fcStudy) return
  fcStudy.index++
  paintFlashcardStudy()
})
el.fcStudyAgain.addEventListener('click', () => resetFlashcardStudy())
el.fcStudy.addEventListener('mousedown', (event) => {
  if (event.target === el.fcStudy) closeFlashcardStudy()
})
document.addEventListener('keydown', (event) => {
  if (el.fcStudy.hidden) return
  if (event.key === 'Escape') { event.preventDefault(); closeFlashcardStudy(); return }
  if (!fcStudy?.answered && /^[1-9]$/.test(event.key)) {
    const index = Number(event.key) - 1
    if (index < el.fcStudyOptions.children.length) {
      event.preventDefault()
      answerFlashcardStudy(index)
    }
  } else if (fcStudy?.answered && event.key === 'Enter' && !el.fcStudyNext.hidden) {
    event.preventDefault()
    el.fcStudyNext.click()
  }
})
...
function keysFor (dir) {
  ...
  const set =
    languageAlphabets.byName[name.trim().toLowerCase()] ||
    languageAlphabets.byName[languageAlphabets.byCountry[countryCode(flag)]] ||
    languageAlphabets.default
...
function paintKeyboard () {
  ...
  if (!keyboard) {
    if (note) {
      ensureLanguageRuntime()
        .then(() => { if (viewingLanguageTable() && !reading()) paintKeyboard() })
        .catch((error) => reportFailure('language keyboard', error))
    }
    return
  }
  keyboard.show(!!note)
...
function updateStatus () {
  ...
  if (state.current && viewingPdf()) {
    const marks = pdf?.marks().length || 0
    text = pdf?.pages()
...
async function openWikilink (target, { newTab = false, side = false } = {}) {
  ...
  if (attachment.path && attachment.kind === 'pdf') {
    ...
    // `pdf` is null while the viewer module is still on its way in — the page
    // is a place in a document that is not on screen yet.
    if (attachment.page && pdf) pdf.goToPage(attachment.page)
    return
  }
 ...
 */
/* Comparison keys are composed (NFC) before they meet. A `é` is one codepoint
   from a keyboard and two from a file a sync client wrote through the old
   macOS convention, and the two spellings are equal to a reader and unequal to
   `===` — so `[[Café]]` used to resolve or not depending on which machine last
   saved the note it names. Composed here, at the comparisons, and never in the
   paths themselves: a path is opened by its exact bytes. */
const foldName = (text) => String(text || '').toLowerCase().normalize('NFC')

function linkTargetFor (wanted, files = state.files) {
  const asked = foldName(wanted)
  const named = bestLinkTarget(files.filter((f) => foldName(f.name) === asked))
  if (named) return named
  const byPath = files.find((f) => foldName(f.path.replace(NOTE_EXT, '')) === asked)
...
/* The notes of the vault, by the two things a link can name them with.

   `noteFromName` resolves one `![[…]]`, and the editor calls it for every embed
   in the visible ranges on every keystroke and every scroll — so what was a
   `filter` of the whole file list, then a second `filter` lowercasing every
   name in it, then sometimes a `find` lowercasing every path, ran a few times
   per frame over the whole vault.

   Keyed on the identity of `state.files` rather than a revision counter: that
   array is replaced wholesale whenever the tree changes (see `flatten` in the
   snapshot handler), so a stale index is not reachable. */
let noteIndex = { from: null, byName: new Map(), byPath: new Map() }

function noteLookup () {
  if (noteIndex.from === state.files) return noteIndex
  const byName = new Map()
  const byPath = new Map()
  for (const f of state.files) {
    if (!NOTE_EXT.test(f.path)) continue
    const name = foldName(f.name)
    const same = byName.get(name)
    if (same) same.push(f)
    else byName.set(name, [f])
    /* First wins, matching the `find` this replaces: two notes whose paths
       differ only in case are a tie the flatten order used to break. */
    const key = foldName(f.path.replace(NOTE_EXT, ''))
    if (!byPath.has(key)) byPath.set(key, f)
  }
  noteIndex = { from: state.files, byName, byPath }
  return noteIndex
}
...
function noteFromName (name) {
  ...
  const wanted = foldName(String(name).replace(NOTE_EXT, ''))
  /* Same three-step order as `linkTargetFor` — name, then path, then alias —
     read out of the index instead of filtered out of the vault. `bestLinkTarget`
     still decides between twins, because which twin wins depends on the note
     doing the linking and so cannot be settled when the index is built. */
  const { byName, byPath } = noteLookup()
  const hit = bestLinkTarget(byName.get(wanted) || []) ||
    byPath.get(wanted) ||
    aliasTarget(wanted)
  return hit ? hit.path : null
}

/** The note an alias claims, among the notes only. */
function aliasTarget (wanted) {
  const claimed = state.aliases[wanted]
  if (!claimed?.length) return null
  const { byPath } = noteLookup()
  const matches = []
  for (const path of claimed) {
    const hit = byPath.get(foldName(String(path).replace(NOTE_EXT, '')))
    if (hit) matches.push(hit)
  }
  return bestLinkTarget(matches)
}
...
function applyOutlineCollapse () {
  ...
  paintOutlineFoldToggle()
}

/**
 * Which way the outline's fold-all points, and whether there is anything for
 * it to do — the same contract the tree's keeps. A heading row is foldable
 * only when it has children, so an outline of flat rows leaves the button
 * hidden outright, as does every document the pane cannot map: a PDF's
 * contents rows carry no folds, and the other kinds hold no headings at all.
 */
function paintOutlineFoldToggle () {
  if (!el.outlineFoldAll) return
  const foldable = outlineRows.filter((row) => row.hasChildren)
  const collapsed = collapsedOutlineKeys()
  const anyOpen = foldable.some((row) => !collapsed.has(row.key))
  el.outlineFoldAll.hidden = !outlineOpen() || !foldable.length
  el.outlineFoldAll.classList.toggle('is-shut', !anyOpen)
  const label = anyOpen ? 'Collapse all headings' : 'Expand all headings'
  el.outlineFoldAll.title = label
  el.outlineFoldAll.setAttribute('aria-label', label)
}

/** Every foldable heading shut, or every one open — whichever the map is not. */
function toggleAllOutline () {
  const foldable = outlineRows.filter((row) => row.hasChildren)
  if (!foldable.length) return
  const collapsed = collapsedOutlineKeys()
  const shutting = foldable.some((row) => !collapsed.has(row.key))
  /* The set answers per heading, so "all" is written the same way the reader
     would by hand: shutting adds every foldable key, opening clears them —
     the branch keeps its smaller arrangement either way once it reopens. */
  if (shutting) for (const row of foldable) collapsed.add(row.key)
...
function setPaneBelow (pane, remember = true) {
  /* A tab in the strip only ever chooses; closing the panel is the header's
     ...
     focus with it. */
  const opening = !paneBelow()
  state.paneBelow = PANES[pane] && sidebarPaneAvailable(pane) ? pane : null
  if (remember) setSetting('paneBelow', state.paneBelow || undefined)
  /* The divider's height is the split's own, not the pane's: choosing a
     different tab leaves it where it is. A saved height is read back only
     when the panel reappears — the live --pane-below already carries it
     through every switch. */
  if (state.paneBelow && opening) setPaneBelowHeight(savedPaneBelowHeight(), false)
  renderPanes()
}
...
function renderPanes () {
  ...
  el.paneSplitToggle.setAttribute('aria-label', el.paneSplitToggle.title)
  if (below) {
...
/* Where Expand takes it: most of the sidebar, past the drag's own ceiling —
   a drag that could reach 90% would leave the tree two rows tall too easily,
   while a chosen expansion says exactly that. */
const PANE_BELOW_EXPANDED = 0.88

function savedPaneBelowHeight () {
  return Number(state.cfg.paneBelowHeight) || PANE_BELOW_DEFAULT
}

function paneBelowShare () {
  return parseFloat(el.sidebar.style.getPropertyValue('--pane-below')) / 100 || PANE_BELOW_DEFAULT
}

function setPaneBelowHeight (share, remember = true) {
  const held = Math.min(PANE_BELOW_EXPANDED, Math.max(PANE_BELOW_MIN, share))
  el.sidebar.style.setProperty('--pane-below', `${held * 100}%`)
  el.paneSplitGrip.setAttribute('aria-valuenow', String(Math.round(held * 100)))
  if (remember) {
    state.cfg = { ...state.cfg, paneBelowHeight: held }
    api.config.set({ paneBelowHeight: held })
  }
}

/* The share Expand left behind — what Collapse puts back. In memory rather
   than config: expansion is a moment's need, not a shape the panel keeps. */
/** @type {number | null} */
let paneBelowHeld = null

/** Expand the lower panel to fill the sidebar, or put back what it had. */
function togglePaneBelowExpand () {
  if (!paneBelow()) return
  if (paneBelowShare() >= PANE_BELOW_EXPANDED - 0.01 && paneBelowHeld !== null) {
    setPaneBelowHeight(paneBelowHeld)
    paneBelowHeld = null
  } else {
    paneBelowHeld = paneBelowShare()
    setPaneBelowHeight(PANE_BELOW_EXPANDED, false)
  }
}
...
el.paneSplitGrip.addEventListener('pointerdown', (event) => {
  ...
  const shareAt = (clientY) => {
    ...
    /* Measured from the bottom, because that is the edge the lower panel owns.
       A drag stays under PANE_BELOW_MAX; the wider room above it is Expand's,
       which a pointer could only reach by leaving the tree two rows tall. */
    return box.height ? Math.min(PANE_BELOW_MAX, (box.bottom - clientY) / box.height) : PANE_BELOW_DEFAULT
  }
...
function fitPaneBelow () {
  const pane = paneBelow()
  const body = pane ? PANES[pane]?.body() : null
  const height = el.sidebar.getBoundingClientRect().height
  if (!body || !height) return
  setPaneBelowHeight(body.scrollHeight / height)
}

el.paneSplitGrip.addEventListener('dblclick', fitPaneBelow)

el.paneSplitGrip.addEventListener('keydown', (event) => {
  if (event.key === 'Enter') { togglePaneBelowExpand(); event.preventDefault(); return }
  const step = event.shiftKey ? 0.08 : 0.02
  const now = paneBelowShare()
...
/** Whether two heading lists say the same thing, in the same order. */
function sameHeadings (a, b) {
  if (a.length !== b.length) return false
  for (let i = 0; i < a.length; i++) {
    if (a[i].level !== b[i].level || a[i].line !== b[i].line || a[i].text !== b[i].text) {
      return false
    }
  }
  return true
...
function renderOutline () {
  ...
  paintOutlineTab()
  /* So is the fold-all beside it — and when the outline is not the pane on
     show, hiding it here is what lets the branches below stay silent about
     a control they never reach. */
  paintOutlineFoldToggle()
  if (!outlineOpen()) return
  ...
  /* Compared heading by heading rather than by joining them all into one
     ...
     used to build was proportional to the note — so the check for "has anything
     changed?" allocated a copy of every heading in the document, on a note
     where the honest answer is nearly always no. */
  if (sig === outlineSig && sameHeadings(next, outlineHeadings) &&
      el.outlineList.childElementCount) {
...
/* ------------------------------------------------- highlights into notes

   A highlight is a quotation whose source is a place in a document. Keeping it
   as a note keeps both halves: the passage as a blockquote, and the place as
   the same `[[doc.pdf#page=N]]` link the embed syntax already understands, so
   the note can always walk back to the paper it came from. */

/**
 * The Markdown a passage is saved as: the quote, then the link back to the
 * page it was read on.
 */
function pdfMarkBlock (mark, pdfPath) {
  const page = mark.rects?.[0]?.page || 1
  const quote = String(mark.text || '')
    .split('\n').map((line) => `> ${line}`).join('\n')
  return `${quote}\n\n— [[${pdfPath}#page=${page}]]`
}

/** The note a PDF's passages gather into by default: a companion beside it. */
function pdfHighlightsPath (pdfPath) {
  const dir = pdfPath.split('/').slice(0, -1).join('/')
  const base = (pdfPath.split('/').pop() || 'document').replace(/\.pdf$/i, '')
  return `${dir ? `${dir}/` : ''}${base} highlights.md`
}

/**
 * Append the mark to `path` — creating the file when it is not there yet, so
 * "the highlights note" and "any note I pick" are one operation.
 *
 * The write carries the stamp of the version read: a note that moved while
 * the picker was open is refused rather than clobbered, and the user is told
 * instead of being handed a silent merge.
 */
async function appendMarkToNote (mark, path) {
  const block = pdfMarkBlock(mark, state.current?.path || '')
  let head = ''
  let expect = null
  try {
    const got = await api.file.readStamped(path)
    ...
    /* Absent or unreadable-as-text. A missing file is the create half of this
       action; anything else (a refused path, a binary) stops it — writing
       into a file we could not read would blindside whatever it holds. */
    const exists = state.files.some((item) => item.path === path)
    if (exists) { toast(`Could not read ${docLabel(path)}.`); return }
  }
  const body = head.trimEnd()
  const text = `${body}${body ? '\n\n' : ''}${block}\n`
  try {
    const wrote = await api.file.write(path, text, expect ? { expect } : null)
    if (wrote?.stale || wrote === false) {
      toast(`${docLabel(path)} changed while the passage was being added — nothing was written.`)
      return
    }
    toast(`Added to ${docLabel(path)}.`)
  } catch (error) {
    reportFailure('saving the passage', error)
  }
}

/** The mark as a card draft: the passage becomes the question the reader
    shapes, and the explanation keeps where it came from. The path is taken
    before the bank opens — afterwards `state.current` is the bank, not the
    paper. */
function openMarkAsFlashcard (mark, bankPath) {
  const page = mark.rects?.[0]?.page || 1
  const pdfPath = state.current?.path || ''
  openNote(bankPath).then(() => {
    openFlashcardComposer(null, {
      question: mark.text || '',
      explanation: `From [[${pdfPath}#page=${page}]]`
    })
  }).catch((error) => reportFailure('opening the bank', error))
}
...
function renderPdfOutline () {
  ...
  /* `openViewed` claims the tab before the route-specific viewer has loaded.
     The outline is painted during that hand-off, while `ensurePdf` may still
     be fetching the chunk; keep the panel honest without dereferencing a
     viewer that does not exist yet. The viewer's onDoc callback redraws it. */
  if (!pdf) {
    const loading = document.createElement('p')
    loading.className = 'outline-empty'
    loading.textContent = 'Opening the PDF…'
    el.outlineList.append(loading)
    return
  }
    ...
    for (const mark of marks) {
      ...
      row.addEventListener('contextmenu', (event) => {
        ...
        renderContextMenu([
          ...
          /* A passage is only worth keeping where it can be found again —
             the note gets the quote and the page link, the card the question. */
          { label: 'Save to a note…', run: () => openOverlay('append-to', { mark }) },
          { label: 'Make a flashcard…', run: () => openOverlay('flashcard-to', { mark }) },
          {
...
/* How many notes a section draws before it stops and offers the rest.

   An index note — the kind a vault grows on purpose — is mentioned by hundreds
   or thousands of others, and this panel built a button for every one of them
   and another for every mention inside them, then threw the lot away and did it
   again on every vault change. Nobody reads the four hundredth backlink without
   asking; the ones who do ask get them. */
const LINKS_SHOWN = 50

/* The sections the reader has asked to see in full, by the note they are
   about — so it lasts while that note is open and is not carried to the next
   one, where the same section means something else. */
let linksExpanded = { path: null, kinds: new Set() }

function linksShowAll (kind) {
  return linksExpanded.path === state.current?.path && linksExpanded.kinds.has(kind)
}

/**
 * The "and N more" foot of a capped section.
 *
 * Expanding redraws the panel rather than appending to it: `renderLinks` is
 * already the one thing that knows how a section is built, and a second, subtly
 * different builder for the tail is how the two drift apart.
 */
function linksMore (kind, hidden) {
  const more = document.createElement('button')
  more.type = 'button'
  more.className = 'link-more'
  more.textContent = `Show ${hidden} more`
  more.dataset.linkMore = kind
  return more
}
...
function outgoingSection (targets) {
  ...
  const all = linksShowAll('outgoing')
  const shown = all ? targets : targets.slice(0, LINKS_SHOWN)
  for (const target of shown) {
    ...
    /* What the row means, written on the row: the panel's one delegated
       listener reads it back. A closure per row is a listener per row, and on
       an index note that was thousands of them, discarded and rebuilt on every
       vault change. */
    if (target.missing) name.dataset.linkCreate = target.target
    else name.dataset.linkOpen = target.path
    section.append(name)
  }
  if (shown.length < targets.length) {
    section.append(linksMore('outgoing', targets.length - shown.length))
  }
  return section
}
...
function linkSection (title, notes, kind) {
  ...
  const all = linksShowAll(kind)
  const shown = all ? notes : notes.slice(0, LINKS_SHOWN)
  for (const note of shown) {
    ...
    /* The mention to jump to, written on the row for the panel's delegated
       listener — see `outgoingSection` for why these are not closures. */
    name.dataset.linkMention = note.path
    if (note.hits[0]) {
      name.dataset.linkLine = String(note.hits[0].line)
      name.dataset.linkCol = String(note.hits[0].col || 0)
    }
    section.append(name)
    ...
    for (const hit of note.hits) {
      ...
      row.dataset.linkMention = note.path
      row.dataset.linkLine = String(hit.line)
      row.dataset.linkCol = String(hit.col || 0)
      section.append(row)
    }
  }
  if (shown.length < notes.length) {
    section.append(linksMore(kind, notes.length - shown.length))
  }
  return section
}
...
async function openMention (path, hit) {
  ...
  revealInTree(path)
  if (hit) goToLine(Math.min(hit.line, noteLines()), hit.col || 0)
}

/* One listener for the whole panel, reading what each row says it means.

   The rows are replaced wholesale on every redraw, so a listener bound to a row
   is a listener bound and dropped on every vault change — thousands of them on
...
el.linksList.addEventListener('click', async (e) => {
  ...
  const d = row.dataset
  if (d.linkMore) {
    if (linksExpanded.path !== state.current?.path) {
      linksExpanded = { path: state.current?.path || null, kinds: new Set() }
    }
    linksExpanded.kinds.add(d.linkMore)
    renderLinks()
    return
  }
  if (d.linkCreate != null) { openWikilink(d.linkCreate); return }
  if (d.linkOpen != null) {
    if (!await openNote(d.linkOpen)) return
    revealInTree(d.linkOpen)
    return
  }
  const hit = d.linkLine == null ? null : { line: Number(d.linkLine), col: Number(d.linkCol || 0) }
  openMention(d.linkMention, hit)
})
...
/* Every word this session has had a verdict for, true when the dictionary does
   not know it. Words are a closed set that a note returns to constantly, so
...
const spellingScanner = makeLineScanner()
...
async function renderSpelling () {
  ...
  /* Nothing has moved since the last pass. This is reached from more than
     typing — a view switch, `ensureEditor` arriving, a pane opening — and each
     of those used to pay for a full copy of the document, a walk of its syntax
     tree and three scans over it to arrive at the answer already on screen. */
  const doc = editor.state.doc
  if (spellingDoc === doc && paneOpen('spelling') === spellingPaneWas) return
  spellingDoc = doc
  ...
  const text = docText(doc)
  /* The word list first, and nowhere any of them is. Working out where every
     word in the note sits — two numbers and an object each — was most of what a
     pass cost, and all but a handful of them are spelled correctly and will
     never be asked for. `places` below fetches the few that are. */
  const distinct = spellingScanner.scan(text, editor.state)
  ...
  /* Only the words this session has not had an answer for yet. A pass sends
     every distinct word in the note, and between two keystrokes that is the
     same several thousand words as last time, for the sake of the one that
     changed — a payload across the process boundary and a dictionary lookup
     each, to be told again what was already known. The dictionary can change
     under this, and when it does the cache is dropped whole. */
  const unknown = asked.filter((word) => !spellKnown.has(word))
  let flagged
  try {
    if (unknown.length) {
    ...
    /* The document is unchanged, so without this the guard above would answer
       every later pass with the same early return and the pane would keep the
       failure on screen long after the dictionary came back. */
    spellingDoc = null
  ...
  /* Now, and only for the words the dictionary objected to: where they are.
     Set before the early returns below — the rows may not be rebuilt, and a
     click has to land where the word is now rather than where it was two edits
     ago. */
  const groups = spellingScanner.places(flagged)
  spellingGroups = groups

  const rows = [...groups.entries()].sort((a, b) => a[1].at[0].from - b[1].at[0].from)
...
/* The Info pane is src/info-pane.js — what the sidebar says about the open
   document, and the two editors that write back to it: the note's own YAML
   head, and the tags of a file that has no head to keep them in. It is the one
   pane that edits rather than draws, which is why it is a module of its own. */


/* ------------------------------------------------------- rendered-HTML memo
   A note's rendered HTML, held against the note's own text string. `noteText`
   hands back the same string for as long as the document is unchanged, so a
   pointer test is the whole invalidation story: edited text is a new string,
   and a new string misses. `equationsFor` is stable per version the same way.

   What this buys is the round trip. Opening note B after A renders B; coming
   back to A used to render it again — about 21ms of markdown-it on a 3,250-line
   note, measured, half of what the whole open costs — to build a page this app
   had already built. With the memo the return trip skips straight to the HTML
   parse: 49.8ms → 26.6ms whole-open on the bench note, and the win scales with
   the note because md.render is linear in it.

   Four entries, not one: tab flippers hold more than two notes, and each entry
   is only the HTML string (~1.7MB for the bench note) rather than a DOM tree.
   The cut is checked rather than trusted — `readingWhole` can flip for a path
   between two calls with the same body — and a miss re-renders into the same
   shape the direct call below would have produced.

   `dir` is checked for the same reason: the render is NOT a pure function of
   the text. `createMarkdown`'s resolveEmbedSrc reads `state.current?.dir` to
   resolve relative `<img src>` in raw HTML, so two identically-bodied notes in
   different folders must not share an entry, and a note whose folder changes
   under it (rename) must miss. */
const RENDER_MEMO_MAX = 4
const renderMemo = new Map()

function memoRender (body, equations, cut) {
  const dir = state.current?.dir || ''
  const hit = renderMemo.get(body)
  if (hit && hit.equations === equations && hit.cut === cut && hit.dir === dir) {
    // Re-inserted so the map itself records recency; eviction takes the oldest.
    renderMemo.delete(body)
    renderMemo.set(body, hit)
    return hit.html
  }
  const html = md.render(cut >= body.length ? body : body.slice(0, cut), { equations })
  if (renderMemo.size >= RENDER_MEMO_MAX) renderMemo.delete(renderMemo.keys().next().value)
  renderMemo.set(body, { equations, cut, dir, html })
  return html
}
...
function scheduleReadingWarmup () {
  cancelReadingWarmup()
  if (!editor || !state.current || reading() || viewingPdf() || viewingSite() || viewingWhiteboard()) return
...
function stopReadingHighlights ({ reset = false } = {}) {
  readingHighlightRoot?.removeEventListener('contentvisibilityautostatechange', onReadingHighlightState)
...
function renderReading ({ reuse = false } = {}) {
  ...
  title.id = 'reading-document-title'
  ...
  col.append(title)
  /* `article` alone is exposed inconsistently by macOS when it is the
     focusable scroller. Name the document from the heading the reader sees so
     VoiceOver enters it as one coherent Reading surface rather than skipping
     from the toolbar to the next pane. */
  el.reading.setAttribute('aria-labelledby', title.id)
  ...
  rendered.innerHTML = memoRender(body, noteEquations(), cut)
  enhanceFlashcards(rendered)
...
function startReadingHighlights (root) {
  /* Which blocks are near the viewport is a question the page is already
     answering: every `.code-wrap` is `content-visibility: auto`, and the
     engine tells each one when it becomes relevant or skipped. That is the
     whole of the trigger here — no observer of our own, and nothing measured.

     It replaced an IntersectionObserver over every block. An observer's cost
     is per target, on every frame in which layout ran — a scroll, a panel
     sliding, a column being resized — and a saved website rendered as a note
     has thousands of fenced blocks; watching all of them was fifteen
     milliseconds of intersection arithmetic per frame before a line was
     drawn. The engine's own relevance tracking is paid for already by the
     content-visibility the page relies on, and this only listens to it. */
  const canListen = 'onvisibilitychange' in document && typeof ContentVisibilityAutoStateChangeEvent === 'function'
  ...
  for (const wrap of root.querySelectorAll('.code-wrap')) {
    ...
    if (!canListen) {
      ...
      // every block uncoloured when the event is unavailable.
  ...
  if (!canListen || readingHighlightRoot === root) return
  readingHighlightRoot?.removeEventListener('contentvisibilityautostatechange', onReadingHighlightState)
  readingHighlightRoot = root
  root.addEventListener('contentvisibilityautostatechange', onReadingHighlightState)
}

/** A block became relevant, or stopped being: colour it, or give the colour
 *  back. The event bubbles, so one listener on the page hears every block. */
function onReadingHighlightState (event) {
  const job = readingHighlightJobs.get(event.target)
  if (!job) return
  job.visible = !event.skipped
  if (!job.visible) {
    resetReadingHighlight(job)
    return
  }
  if (job.pending || job.highlighted) return
  job.pending = true
 ...
 */
/* The addressable blocks of the rendered note, collected once per render.

   The bisection below was fixed long ago; the collection above it was not.
   `querySelectorAll('[data-line]')` is an unindexed attribute match over the
   whole rendered note that materialises a static list of every block in it, and
   this runs once per scroll frame — through `queueOutlineMark` — and again on
   every `markPlace`. On the 1.7MB note the comment below is written about, that
   is ~50,000 node references allocated per frame, to answer a question whose
   answer changes only when the note is re-rendered.
...
function readingLineNodes () {
  /* The `isConnected` guard is for a rebuild that forgot to say so: a detached
     list measures as a column of zeros, which is silently wrong rather than
     loudly broken, and that is the failure worth paying one property read to
     rule out. */
  if (!readingLines || (readingLines.length && !readingLines[0].isConnected)) {
    readingLines = el.reading.querySelectorAll('[data-line]')
  }
  return readingLines
}
...
function restoreReadingPlace (line, { center = false } = {}) {
  ...
  const settle = () => {
    ...
    const frame = pane.getBoundingClientRect()
    /* To the top of the pane, or to its middle: a bookmark is shown with the
       page around it rather than as the first thing under the toolbar. */
    const off = box.top - frame.top - (center ? Math.max(0, (frame.height - box.height) / 2) : 0)
    ...
    if (Math.abs(off) > 0.5) pane.scrollTop += off
    mine = pane.scrollTop
    requestAnimationFrame(settle)
  }
  settle()
...
function updateViewControl () {
  ...
  /* The switch said out loud: pressing a button announces the press but not
     the mode it moved to, so a change is spoken here — but only a change. The
     boot-time pass that marks the resting view announces nothing. */
  if (announcedView && announcedView !== state.view) {
    el.viewAnnounce.textContent = `${VIEW_NAMES[state.view]} view`
  }
...
const COMMANDS = [
  ...
  { id: 'open-beside', title: 'Open document beside this one', scope: 'file', keywords: 'side pane split second reference' },
  ...
  { id: 'manage-packages', title: 'Manage code packages…', scope: 'markdown', keywords: 'python rust javascript typescript node go julia pip npm install dependencies environment' },
    ...
    { id: 'insert-citation', title: 'Insert citation…', scope: 'markdown', keywords: 'cite reference bibliography bibtex paper' },
  ...
  { id: 'review-stats', title: 'Review statistics…', scope: 'study' },
    { id: 'study-all', title: 'Study all due words', scope: 'studywords' },
    { id: 'study-flashcards', title: 'Study all due flashcards', scope: 'studyfc', keywords: 'quiz cards review bank' },
    { id: 'import-cards', title: 'Import cards from CSV…', scope: 'language' },
    { id: 'export-cards', title: 'Export cards as CSV…', scope: 'language', keywords: 'anki spreadsheet deck vocabulary' },
...
function commandList () {
  ...
  const SCOPES = {
    ...
    language: () => viewingLanguageTable(),
    /* The vault-wide study commands describe data that outlives the open
       document — a vault holding tables or banks owes them wherever the user
       is standing, not only over the file that happens to be open. */
    studywords: () => viewingLanguageTable() ||
      state.files.some((item) => isLanguageTablePath(item.path)),
    studyfc: () => viewingFlashcardBank() ||
      state.files.some((item) => isFlashcardBankPath(item.path)),
    study: () => viewingLanguageTable() || viewingFlashcardBank() ||
      state.files.some((item) => isLanguageTablePath(item.path) || isFlashcardBankPath(item.path)),
  ...
  /* The lower panel's third act — after drag and double-click, the verb a
     keyboard or a palette reader reaches for when a deep outline wants the
     whole sidebar. Only offered while there is a lower panel to expand. */
  if (paneBelow()) {
    commands.push({ id: 'expand-pane', title: 'Expand or collapse the lower panel', keywords: 'sidebar outline links info taller bigger resize' })
  }
  if (viewingWhiteboard()) {
    commands.push(
 ...
 * `{{title}}` is the note being written into, not the template. `prompts` is
 * the answers collected for `{{prompt:Label}}` — see insertTemplate.
 */
function fillTemplate (text, title, prompts = {}) {
  const now = new Date()
  ...
  /* The ISO week: the Thursday of this week fixes both the week number and
     the year it belongs to, which is why a plain "days so far ÷ 7" gets the
     turn of the year wrong. */
  const thursday = new Date(now)
  thursday.setDate(now.getDate() + (3 - ((now.getDay() + 6) % 7)))
  const week = Math.ceil(1 + (thursday.getTime() - new Date(thursday.getFullYear(), 0, 4).getTime()) / (7 * 864e5))
  const values = {
    date: day(0),
    yesterday: day(-1),
    tomorrow: day(1),
    ...
    week: `${thursday.getFullYear()}-W${pad(week)}`,
    title
  }
  return String(text).replace(
    /\{\{\s*(date|yesterday|tomorrow|time|weekday|week|title)\s*\}\}|\{\{\s*prompt\s*:\s*([^}]+?)\s*\}\}/gi,
    (whole, name, prompt) => {
      if (prompt !== undefined) {
        const answer = prompts[prompt.trim().toLowerCase()]
...
/** The `{{prompt:Label}}` questions a template asks, in order, once each. */
function templatePrompts (text) {
  const seen = new Set()
  const out = []
  for (const m of String(text).matchAll(/\{\{\s*prompt\s*:\s*([^}]+?)\s*\}\}/gi)) {
    const label = m[1].trim().toLowerCase()
    if (label && !seen.has(label)) { seen.add(label); out.push(m[1].trim()) }
  }
  return out
}

/**
 * Ask for a template's `{{prompt:…}}` values before it is inserted.
 * Answers with the map, or null when the dialog is cancelled — a cancelled
 * question inserts nothing rather than inserting the braces unanswered.
 */
function askTemplatePrompts (labels) {
  return new Promise((resolve) => {
    const dialog = document.createElement('dialog')
    dialog.className = 'workspace-tools template-prompts'
    const heading = document.createElement('h2')
    heading.textContent = 'This template asks a few things first.'
    const inputs = new Map()
    dialog.append(heading)
    for (const label of labels) {
      const row = document.createElement('label')
      row.textContent = label[0].toUpperCase() + label.slice(1)
      const input = document.createElement('input')
      input.setAttribute('aria-label', label)
      inputs.set(label.toLowerCase(), input)
      row.append(input)
      dialog.append(row)
    }
    const insert = document.createElement('button')
    insert.textContent = 'Insert template'
    const cancel = document.createElement('button')
    cancel.textContent = 'Cancel'
    const finish = (values) => { dialog.close(); resolve(values) }
    insert.onclick = () => finish(Object.fromEntries([...inputs].map(([k, i]) => [k, i.value])))
    cancel.onclick = () => finish(null)
    dialog.addEventListener('cancel', () => resolve(null))
    dialog.append(insert, cancel)
...
async function insertTemplate (path) {
  ...
  const title = state.current?.path
    ...
    : ''
  const labels = templatePrompts(text)
  const prompts = labels.length ? await askTemplatePrompts(labels) : {}
  if (prompts === null) return
  const filled = fillTemplate(text, title, prompts)
...
function openOverlay (mode, meta = {}) {
  ...
  /* At a window narrow enough for the side panels to be drawers, one left
     open sits over this panel — --z-drawer outranks --z-modal — with the
     field it just offered underneath. Opening the panel takes the drawer
     away first, the same trade the Copilot toggle already makes. */
  if (window.innerWidth <= 1040 && el.app.dataset.side === 'open') closeSidePane()
  if (window.innerWidth <= 760 && sidebarOpen()) toggleSidebar(false)
  if (window.innerWidth <= 760 && el.app.dataset.ai === 'open') copilot.close()
  ...
  /* The three pickers whose subject is the page behind them — the theme and the
     two fonts — step aside into the corner and stop covering it, see `.is-peek`.
     The rest stay where they are: they are lists to read, not previews.

     This is what replaced the font specimen card. A page of lorem ipsum beside
     the list answered "what do the letters look like", which is the smaller
     half of the question; the note itself, in the reader's own words at the
     reader's own measure, answers the whole of it. */
  const peek = mode === 'themes' || Boolean(FONT_MODES[mode])
  el.overlay.classList.toggle('is-peek', peek)
  /* And it stops being a modal while it is there. A modal takes the app out of
     the accessibility tree and out of hit-testing — which is right for a dialog
     asking a question, and exactly wrong here: the note under this one is the
     thing being chosen between, and it has to stay scrollable to see. */
  el.panel.setAttribute('aria-modal', String(!peek))
  el.panelInput.value = String(meta.query || '')
  el.panelInput.placeholder = OVERLAY_PROMPT[mode]
  el.panelInput.setAttribute('aria-label', OVERLAY_LABEL[mode] || 'Quick switcher')
  ...
  /* The count lives in the foot but is not part of the key hints above, which
     are rewritten wholesale every time the panel opens — that write was taking
     the live region with it, and a live region that has been replaced announces
     nothing. Put back, as the same element, so the reference held at boot stays
     the one in the document. */
  el.panelCount.textContent = ''
  el.panelFoot.prepend(el.panelCount)
...
function closeOverlay () {
  ...
  else if (viewingData()) dataInstance?.focus()
...
/* Every note as something the switcher can score, built once per tree.

   ⌘O deliberately does not debounce — the list is in memory and the reader is
   meant to see it narrow as they type — so this ran on every keystroke, and it
   allocated a fresh copy of every entry in the vault to add one field to it.
   The entries are read-only here, and `state.files` is replaced wholesale when
   the tree changes, so its identity is the only staleness test needed. */
let switcherCache = { from: null, items: [] }

function switcherItems () {
  if (switcherCache.from !== state.files) {
    switcherCache = {
      from: state.files,
      items: state.files.map((f) => ({ ...f, label: f.name }))
    }
  }
  return switcherCache.items
}

/* The notes a PDF passage can join. Plain Markdown only: a quote block in a
   .fc bank parses as a broken card, and a .lang file is a table, not prose —
   offering either would write something that means nothing where it lands.
   The companion "highlights" note leads the list, and is offered whether or
   not it exists yet: picking a name that is not a file is the create half of
   the action. */
function appendTargets () {
  const items = []
  const companion = viewingPdf() ? pdfHighlightsPath(state.current?.path || '') : ''
  if (companion) {
    const existing = state.files.some((item) => item.path === companion)
    items.push({
      path: companion,
      name: docLabel(companion),
      label: existing ? docLabel(companion) : `${docLabel(companion)} — new`,
      dir: companion.split('/').slice(0, -1).join('/')
    })
  }
  for (const item of state.files) {
    if (item.path === companion) continue
    if (!NOTE_EXT.test(item.path) || isLanguageTablePath(item.path) || isFlashcardBankPath(item.path)) continue
    items.push({ ...item, label: item.name })
  }
  return items
}

/* The banks a flashcard can be made in — `.fc` files are the whole of it. */
function flashcardTargets () {
  return state.files
    .filter((item) => isFlashcardBankPath(item.path))
    .map((item) => ({ ...item, label: item.name }))
}

/* ---------------------------------------------------- the citation picker */

/* Every reference in the note's bibliography, labelled the way a rendered
   citation reads — author and year first, the title after, the key on the row
   for the person who knows it. */
async function openCitationPicker () {
  const bib = state.resolveAsset?.('references.bib', state.current?.dir || '')
  if (!bib) {
    toast('No references.bib beside this note — add one to cite from it.')
    return
  }
  let entries
  try {
    entries = parseBibTeXCached(await api.file.read(bib))
  } catch {
    toast('The bibliography could not be read.')
    return
  }
  if (!entries.size) {
    toast('The bibliography has no entries to cite.')
    return
  }
  openOverlay('cite', { entries })
}

/* `[@key]`, where the caret is. The palette opens from the reading view too,
   so the note is put into its editor first — the same courtesy inserting a
   template already extends. */
async function insertCitation (key) {
  const view = await ensureEditor()
  const at = view.state.selection.main
  const cite = `[@${key}]`
  view.dispatch({
    changes: { from: at.from, to: at.to, insert: cite },
    selection: { anchor: at.from + cite.length },
    scrollIntoView: true
  })
  view.focus()
}
...
async function runOverlayQuery (query) {
  ...
  if (mode === 'switcher' || mode === 'commands' || mode === 'new-files' ||
      ...
      mode === 'append-to' || mode === 'flashcard-to' || mode === 'cite' ||
    ...
    const source = mode === 'vaults'
      ...
      ? flashcardTargets()
      : mode === 'cite'
      ? [...(state.overlay.entries || new Map())].map(([key, entry]) => ({
          /* The key rides as the row's code: typed in full it outranks a
             fuzzy label hit, the way a country code does. */
          id: key, code: key,
          label: `${citationLabel(entry, key)} — ${entry.title || key}`
        }))
      : mode === 'templates'
      ? templateItems()
      : mode === 'switcher'
    ...
    /* Commands at rest lead with what was run lately: the alphabetical list
       answers "what can this do", and the Recent group answers "the thing I
       did a moment ago", which is the commoner question. A recent id that no
       longer applies — or no longer exists — is simply not in `scored`. */
    if (mode === 'commands' && !query) {
      const rank = new Map((state.cfg.recentCommands || []).map((id, i) => [id, i]))
      const recent = scored.filter((s) => rank.has(s.item.id))
      if (recent.length) {
        recent.sort((a, b) => (rank.get(a.item.id) ?? 0) - (rank.get(b.item.id) ?? 0))
        for (const s of recent) s.item.recent = true
        scored = recent.concat(scored.filter((s) => !rank.has(s.item.id)))
      }
    }
  ...
  if (mode === 'search') {
    ...
    if (!query.trim()) {
      state.overlay.items = []
      state.overlay.index = 0
      renderOverlayList('Type to search — note → type:note · pdf → type:pdf · highlight → type:highlight · tag → tag: · folder → path: · file → file: · property → prop:')
      return
    }

    /* The first search of a session builds the vault's index, which on a large
       vault is a walk of every note — and until now the panel sat showing the
       last query's rows with nothing to say it was working. */
    el.panel.classList.add('is-searching')
    el.panelList.setAttribute('aria-busy', 'true')
    let answer
    try {
      answer = await api.search(query, searchOpts)
    } finally {
      // Only the newest search speaks for the panel; an older one finishing
      // afterwards must not declare it idle.
    ...
    /* Main stood this search aside because a newer one had already started —
       ...
       nothing wrong: the search that replaced this one is on its way, and the
       rows on screen belong to the last complete answer. Painting the empty
       list would say "nothing matches" for as long as it took the real answer
       to arrive, which is a flicker of a wrong statement on every keystroke. */
    if (cancelled) return
    ...
    announceResults(state.overlay.items.length, results.length)
    /* The document ⌘ may peek at is parsed and drawn with the results, so
       holding the key over the top row is a swap of a finished picture, not
       a wait for the file to open and rasterise. */
    const peekable = state.overlay.items.find(({ item }) => item.kind === 'pdf' || item.kind === 'highlight')
    if (peekable) {
      const { item: peek } = peekable
      previewPdfDoc(peek.path)
        .then((doc) => peekCanvas(doc, peek.path, Math.min(Math.max(1, peek.hit.page || 1), doc.numPages), peekWidth()))
        .catch(() => {})
    }
  }
}
...
/**
 * How much was found, said once per search.
 *
 * Neither a sighted reader nor a screen reader was told: the rows appeared and
 * the foot carried only the caveats. It lives in a polite live region so that
 * arriving results are announced without interrupting the typing that asked
 * for them — and it is written as one sentence rather than assembled from the
 * row count on screen, which is capped and would understate it.
 */
function announceResults (hits, notes) {
  if (!el.panelCount) return
  el.panelCount.textContent = hits
    ? `${hits} ${hits === 1 ? 'result' : 'results'} in ${notes} ${notes === 1 ? 'note' : 'notes'}`
    : ''
}
...
/**
 * Point the query field at the row that is highlighted.
 *
 * Focus stays in the field — arrowing through a list you cannot Tab into is the
 * whole point of a combobox — so the highlighted row has to be named from
 * there, or a screen reader is left standing on a text box while something it
 * cannot see changes behind it.
 *
 * @param {Element|null} row  the highlighted row, or null to say there is none
 */
...
function renderOverlayList (emptyMessage = 'Nothing matches.') {
  ...
  const CHUNK = 80
  /* A quiet label over a run of rows — the Recent/All commands split. Not a
     `[role="option"]` row: it is not a thing to choose, so it carries no
     index and the arrows and activedescendant never see it. */
  const groupLabel = (text) => {
    const li = document.createElement('li')
    li.className = 'panel-group'
    li.textContent = text
  ...
  const paint = (from) => {
    ...
    for (let i = from; i < stop; i++) {
      const recent = !!items[i].item.recent
      if (recent && (i === 0 || !items[i - 1].item.recent)) frag.append(groupLabel('Recent'))
      if (!recent && i > 0 && items[i - 1].item.recent) frag.append(groupLabel('All commands'))
  ...
  paint(0)
  /* After the first chunk, which always holds the highlighted row: the index is
     0 on every fresh list, and only arrowing moves it — and arrowing goes
     through `syncSelection`, which names it again. */
  nameActiveRow(el.panelList.querySelector('[aria-selected="true"]'))
}
...
function overlayRow ({ item, hits }, i, mode, index) {
  ...
  if (item.recent) li.setAttribute('aria-description', 'recently used')
  ...
  if (mode === 'themes') {
    ...
    right.textContent = mode === 'commands' || mode === 'new-files'
      ...
      : (item.dir || (mode === 'search'
          /* Text kinds say where in the file; page kinds say which page. A
             kind this list has never heard of falls to the text form, not
             the PDF one — a hit with no page number rendered as "p.
             undefined" is how a new kind used to announce itself. */
          ? (item.kind === 'note' || item.kind === 'code' || item.kind === 'data' || item.kind === 'notebook'
              ? `line ${item.hit.line}`
              : item.kind === 'site'
                  // A hit in a website file is in its title or its address,
                  // and "line 1" says nothing about either.
                  ? 'website'
                  : item.kind === 'whiteboard'
                  ? 'whiteboard'
                  : item.kind === 'docx'
                    ? 'Word'
                    : item.kind === 'highlight' || item.kind === 'pdf'
...
/* The two halves of a fast peek: parsed documents and rendered pages, both
   kept on a short leash. A fresh `getDocument` per row parsed the same PDF
   on every pass of the pointer, and a fresh canvas per hover rasterised the
   same page — the lag each one cost was spent on work already done. */
/** @type {Map<string, { loading: any, doc: Promise<any> }>} */
const previewPdfs = new Map()
/** @type {Map<string, HTMLCanvasElement>} */
const previewPages = new Map()
/* Which page the card is already showing: arming back onto the same row can
   unhide it as-is rather than waiting on a render of what is up. */
let previewShownKey = null

function dropPreviewCache () {
  for (const entry of previewPdfs.values()) entry.loading?.destroy().catch(() => {})
  previewPdfs.clear()
  previewPages.clear()
  previewShownKey = null
  peekObserver?.disconnect()
  peekObserver = null
}

async function previewPdfDoc (path) {
  let entry = previewPdfs.get(path)
  if (entry) {
    previewPdfs.delete(path)
    previewPdfs.set(path, entry)
    return entry.doc
  }
  const { loadPdfjs, PDF_DATA } = await import('./pdf.js')
  /** @type {{ loading: any, doc: Promise<any> }} */
  const record = { loading: null, doc: Promise.resolve(null) }
  record.doc = (async () => {
    const [source, lib] = await Promise.all([api.pdf.source(path), loadPdfjs()])
    record.loading = lib.getDocument({ url: source, ...PDF_DATA })
    return record.loading.promise
  })()
  previewPdfs.set(path, record)
  if (previewPdfs.size > 3) {
    const [oldPath, old] = previewPdfs.entries().next().value
    previewPdfs.delete(oldPath)
    old.loading?.destroy().catch(() => {})
  }
  return record.doc
}

/* The page card answers a tap of ⌘ — down and up with nothing in between —
   and stays until the next tap. A chord is not a tap: ⌘C still copies and
   leaves the card alone, which is what makes a bare modifier safe to use as
   a switch. Arming is a search gesture, so a tap with no panel open is no
   gesture at all. */
let searchPreviewArmed = false
let metaHeld = false
let metaChored = false
const armSearchPreview = (armed) => {
  if (searchPreviewArmed === armed) return
  if (armed && !state.overlay) return
  searchPreviewArmed = armed
  previewSearchResult()
}
window.addEventListener('keydown', (e) => {
  if (e.key === 'Meta') { metaHeld = true; metaChored = false; return }
  if (metaHeld) metaChored = true
})
window.addEventListener('keyup', (e) => {
  if (e.key !== 'Meta') return
  metaHeld = false
  if (!metaChored) armSearchPreview(!searchPreviewArmed)
})
window.addEventListener('blur', () => { metaHeld = false; armSearchPreview(false) })

/* The page fills the card's width — the card scrolls, so nothing is squeezed
   to fit a height. A thin scrollbar's worth is taken off so the sheet does
   not slide under it. The width is the one `.is-page` declares. */
const peekWidth = () => Math.min(920, window.innerWidth * 0.92) - 10

/* The rendered bitmap of one page, kept so a re-visit costs nothing. The
   width is in the key: a card resized with the window wants a new bitmap,
   not the old one stretched. */
async function peekCanvas (doc, path, p, width) {
  const key = `${path}#${p}@${width}`
  let canvas = previewPages.get(key)
  if (canvas) {
    previewPages.delete(key)
    previewPages.set(key, canvas)
    return canvas
  }
  const { renderPageToCanvas } = await import('./pdf.js')
  const proxy = await doc.getPage(p)
  const base = proxy.getViewport({ scale: 1 })
  ;({ canvas } = await renderPageToCanvas(proxy, width / base.width))
  previewPages.set(key, canvas)
  if (previewPages.size > 16) previewPages.delete(previewPages.keys().next().value)
  return canvas
}

/* The live observer: nothing renders until a page scrolls into reach. */
/** @type {IntersectionObserver | null} */
let peekObserver = null

/**
 * The card as a small reader: the hit page first, then every other page as
 * it scrolls into reach. Slots carry the page's shape from the start so the
 * scroll bar is truthful before anything is drawn.
 */
async function paintPdfPeek (item) {
  const doc = await previewPdfDoc(item.path)
  const hit = Math.min(Math.max(1, item.hit.page || 1), doc.numPages)
  const width = peekWidth()
  const hitBase = (await doc.getPage(hit)).getViewport({ scale: 1 })
  const stack = document.createElement('div')
  stack.className = 'peek-pages'
  const slots = []
  for (let p = 1; p <= doc.numPages; p++) {
    const slot = document.createElement('div')
    slot.className = 'peek-slot'
    slot.style.aspectRatio = `${hitBase.width} / ${hitBase.height}`
    slot.dataset.page = String(p)
    stack.append(slot)
    slots.push(slot)
  }
  searchPreview.replaceChildren(stack)
  const generation = searchPreviewGeneration
  const renderPeekPage = async (slot, p) => {
    const canvas = await peekCanvas(doc, item.path, p, width)
    if (generation !== searchPreviewGeneration) return
    slot.style.aspectRatio = `${canvas.width} / ${canvas.height}`
    canvas.className = 'peek-page'
  ...
  const io = new IntersectionObserver((entries) => {
    for (const e of entries) {
      ...
      renderPeekPage(e.target, Number(/** @type {HTMLElement} */ (e.target).dataset.page)).catch(() => {})
    }
  }, { root: stack, rootMargin: '150%' })
  peekObserver = io
  for (const slot of slots) io.observe(slot)
  /* The page asked for first and on its own — the observer's margin is for
     the pages scrolled towards, not the one being read now. */
  io.unobserve(slots[hit - 1])
  await renderPeekPage(slots[hit - 1], hit)
  if (generation !== searchPreviewGeneration) return
  searchPreview.hidden = false
  slots[hit - 1].scrollIntoView({ block: 'start' })
  previewShownKey = item.path
}
...
function previewSearchResult () {
  ...
  const page = !!item && (item.kind === 'pdf' || item.kind === 'highlight')
  /* A text excerpt is cheap enough to answer on every selection; a page is
     not — it renders only while ⌘ is held. */
  const show = !!item && (!page || searchPreviewArmed)
  searchPreview.classList.toggle('is-page', show && page)
  /* The page card is a sheet over the panel rather than a second column in
     it; the text excerpt is still what widens it. */
  el.panel.classList.toggle('has-search-preview', show && !page)
  if (!item) {
    searchPreview.hidden = true
    dropPreviewCache()
    return
  }
  if (!show) { searchPreview.hidden = true; return }
  if (page) {
    /* Already showing this document's stack: arming back onto it is a jump
       to the page the row names, not a rebuild. */
    if (previewShownKey === item.path) {
      searchPreview.hidden = false
      const hit = Math.min(Math.max(1, item.hit.page || 1), 9999)
      searchPreview.querySelector(`[data-page="${hit}"]`)
        ?.scrollIntoView({ block: 'start' })
      return
    }
    /* The card appears whole: a "Loading…" first paint made every peek read
       as two flashes — text, then the page. It stays hidden until the hit
       page is drawn. */
    searchPreview.hidden = true
    peekObserver?.disconnect()
    searchPreviewTimer = setTimeout(async () => {
      try {
        await paintPdfPeek(item)
      } catch (error) {
        if (generation !== searchPreviewGeneration) return
        previewShownKey = null
        searchPreview.textContent = error.message
        searchPreview.hidden = false
      }
    }, 50)
    return
  ...
  searchPreviewTimer = setTimeout(async () => {
    try {
      const passage = ['note', 'code'].includes(item.kind)
...
async function chooseOverlayItem (i) {
  ...
  if (!entry) return
  const { mode, dir, paths, mark } = state.overlay
  const { item } = entry
  closeOverlay()

  if (mode === 'move-to') { await moveInto(item.path, paths); return }
  if (mode === 'append-to') { await appendMarkToNote(mark, item.path); return }
  if (mode === 'flashcard-to') { openMarkAsFlashcard(mark, item.path); return }
  if (mode === 'cite') { await insertCitation(item.id); return }
  ...
  if (mode === 'search' && item.hit) {
    /* Same gap as the one `goToCitation` steps over: the first PDF of a session
       claims its tab while the viewer module is still being imported, and a hit
       activated in that moment has nothing to drive yet. */
    if (item.kind === 'highlight' && item.hit.mark) {
      if (pdf) pdf.goToMark(item.hit.mark)
      return
    }
    if (item.kind === 'pdf') {
    ...
    /* A table and a notebook open in panes of their own, which the editor's
       caret cannot be driven into — `goToLine` below speaks CodeMirror, and
       aiming it at whatever buffer the editor last held would scroll a
       document that is not on screen. A source file falls through: it opens
       in the editor, so a line number means what it says. */
    if (item.kind === 'data' || item.kind === 'notebook') return
    // Nor a website: the file's two lines are not what is on screen, and the
    // page they name has no caret to move.
...
function explorerCreateItems (dir = '', insideFolder = false) {
  ...
  return [
    { label: `New markdown file${suffix}`, run: () => createNote(dir) },
    { label: `New flashcard bank${suffix}`, run: () => createFlashcardBank(dir) },
    ...
    { label: `New source file${suffix}`, run: () => openOverlay('new-source', { dir }) },
...
function renderContextMenu (items, event) {
  ...
  for (const item of items) {
    ...
    if (item.key) {
      ...
      k.className = 'key'
      k.textContent = keyLabel(item.key)
      btn.append(k)
    }
...
async function removeEmbeddedImage (path) {
  ...
  try {
    ...
    toast(reason(err, 'That image could not be moved to the Trash.'))
    return
  }
...
const GETTING_STARTED = `# Getting started with Tulip

Tulip is a local-first workspace. Your vault is an ordinary folder, and your notes remain portable Markdown files.

## Write a note

Press \`Cmd/Ctrl+N\` to make a note. Press \`Cmd/Ctrl+2\` to edit, \`Cmd/Ctrl+1\` to read, and \`Cmd/Ctrl+3\` to inspect the raw Markdown.

Link notes with \`[[Note name]]\`. A missing link creates a note in the same folder. Use the Outline, Links, and Info panels in the sidebar to move around a document.

## Keep research together

Drop PDFs, images, websites, notebooks, or Word documents into the vault. Select text in a PDF and use the highlighter; its marks stay with the vault.

## Use the command palette

Press \`Cmd/Ctrl+P\` for commands and \`Cmd/Ctrl+O\` to jump to a file. The palette also contains templates, exports, tags, backups, and study tools.

## Study a language

Create a language table, add a Word and English column, then press Study. With a language table open, use \`Study all due words\` from the command palette for a daily review across the whole vault.

## Run code carefully

Runnable code and notebook cells can use files and the network. Tulip asks once before code runs in a vault. Only trust a vault containing notes you wrote or reviewed.

## Optional Copilot

Open Copilot with \`Cmd/Ctrl+Shift+A\`. Start in Read mode; Ask mode lets it edit after confirmation, and its changes can be reviewed or restored from History.
`

async function openGettingStarted () {
  if (!state.vault) return connectVault()
  const existing = state.files.find((file) =>
    NOTE_EXT.test(file.path) && file.path.replace(NOTE_EXT, '').toLowerCase() === 'getting started')
  let path = existing?.path
  try {
    if (!path) {
      path = await api.file.create('', 'Getting Started')
      const result = await api.file.write(path, GETTING_STARTED)
      if (result?.ok === false) throw new Error(result.error || 'The guide could not be written.')
      await loadTree()
    }
    await openNote(path)
    revealInTree(path)
  } catch (error) {
    toast(reason(error, 'The Getting Started note could not be opened.'))
  }
}
...
async function createNote (dir = '') {
  ...
  // to type on it.
  if (reading()) setView('edit')
  nameNewRow({ type: 'file', path, name: 'Untitled' })
}
...
/**
 * The page on screen, kept as a note beside the file that points at it.
 *
 * Reading the web and writing about it is the whole reason a `.website` file
 * is in the vault at all, and until now the two halves could not be joined:
 * the page's words lived in a guest, so quoting them meant selecting by hand
 * through a window that scrolls. What is written is the page's text, under its
 * title, with the address it came from — a source line rather than a silent
 * copy, because a clipping with no provenance is the one kind nobody can
 * check later.
 */
async function clipPage () {
  ...
  const dir = state.current?.path?.includes('/')
    ...
    : ''
  /* A page title is not a filename: it carries colons, slashes and pipes, all
     of which main would refuse (see electron/safe-name.js) — and being told
     "that name is illegal" about a name the reader never typed is not a
     failure they can act on. Cleaned here so the clipping simply works. */
  const name = (page.title || '')
    .replace(/[\\/<>:"|?*\u0000-\u001f]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 80)
    .replace(/[.\s]+$/, '') || 'Clipped page'
  let path
  try { path = await api.file.create(dir, name) } catch (err) { toast(reason(err, 'That note could not be created.')); return }
  ...
  const body = [
    ...
    '',
    page.text,
    page.truncated ? '\n*(The rest of the page was longer than a clipping keeps.)*' : ''
  ].join('\n')
  try {
    await api.file.write(path, body)
  } catch (err) {
    toast(reason(err, 'That note could not be written.'))
    return
  }
  await loadTree()
  await openNote(path, { newTab: true })
  revealInTree(path)
}
...
async function createWhiteboard (dir = '') {
  ...
  await loadTree()
  await openNote(path)
  nameNewRow({ type: 'file', kind: 'whiteboard', path, name: 'Untitled' })
}
...
async function createTex (dir = '') {
  ...
  await loadTree()
  await openNote(path)
  nameNewRow({ type: 'file', kind: 'tex', path, name: 'Untitled' })
}
...
async function createSource (dir = '', ext = '.txt') {
  ...
  try {
    ...
    toast(reason(err, 'That file could not be created.'))
    return
  }
  if (dir) state.expanded.add(dir)
  await loadTree()
  ...
  if (reading()) setView('edit')
  nameNewRow({
    type: 'file', kind: noteRef(path).kind, path,
    name: isFlashcardBankPath(path) ? 'Untitled' : `Untitled${ext}`
  })
}

/** A portable .fc bank, seeded with only its file-type frontmatter. */
function createFlashcardBank (dir = '') { return createSource(dir, '.fc') }
...
async function createFolder (dir = '') {
  ...
  await loadTree()
  nameNewRow({ type: 'folder', path, name: path.split('/').pop() })
}
...
async function renameNote (node, next) {
  ...
  try {
    ...
    toast(reason(err, 'That name is already taken.'))
...
/**
 * Put a just-created row into its inline rename.
 *
 * Two things have to be true before the editor can open, and `beginRename`
 * checks neither: the Files pane has to be showing, and every folder on the
 * way to the new row has to be unfolded. Miss either and the row is not in the
 * document — the lookup answers nothing, the rename is silently skipped, and
 * the file arrives called "Untitled", or the folder "New folder", with no
 * offer to say what it really is. Creating something is also the moment you
 * are told where it went, and revealing it is that same scroll.
 */
function nameNewRow (node) {
  showInFileExplorer(node.path)
  const row = el.tree.querySelector(`.row[data-path="${cssEscape(node.path)}"]`)
  if (!row) return
  const input = beginRename(node, row)
  if (!input) return
  /* Opening the new document puts the caret in the editor, and not all of that
     lands in this tick: the name field came up selected and then quietly lost
     the typing to the page behind it, so a new file stayed "Untitled" unless
     you noticed and clicked the field. The caret is claimed back a frame
     later, once the document has finished taking it — and only if nothing else
     has been given it meanwhile, so a reader who has already clicked into the
     text is not dragged back out. */
  requestAnimationFrame(() => {
    if (!input.isConnected || document.activeElement === input) return
    if (document.activeElement?.closest?.('.row-input, input, textarea')) return
    input.focus()
    input.select()
  })
}
...
async function showVaultHealth () {
  ...
  try {
    ...
    if (!tab) {
      tab = { path: null, history: [], historyAt: -1, base: null, pinned: false, memory: { kind: 'vault-health', title: 'logs.md', text: '' } }
      const at = state.tabs.length ? state.tabIndex + 1 : 0
      state.tabs.splice(at, 0, tab)
    }
...
function closeCurrentNote () {
  ...
  /* Closing is leaving. `leaveDoc` gives the claim up on the way to another
     document, but this is the way out of one to nothing at all — the last
...
async function removeNode (node) {
  ...
  try {
    ...
    toast(reason(err, 'That item could not be moved to the Trash.'))
    return
  }
...
/** The toolbar's own Run, pressed from the keyboard. The button is the one
 *  arbiter of whether the file on screen can run — applyPanes shows it only
 *  over a runnable source file — so the command defers to it entirely. */
function runCurrentFile () {
  if (el.docRun && !el.docRun.hidden && !el.docRun.disabled) { el.docRun.click(); return }
  toast('Nothing to run — open a runnable source file first.')
}
...
async function runAllCodeBlocks () {
  ...
  try {
    const { ran, failed, stopped, denied } = await runBlocksInOrder(blocks, (done, total) => {
    ...
    if (denied) {
      toast('Code execution was not approved for this vault.')
    } else if (stopped) {
...
/**
 * A notebook command, or the reason it did nothing.
 *
 * These items are always enabled — the menu is built once at launch and would
 * have to be rebuilt on every tab switch to grey them out per document — so
 * choosing one over a note is a thing that happens, and it used to happen in
 * total silence. Only reachable by pointing at the menu, which is what makes
 * the notice right: nobody presses "Run All" by accident.
 */
function onNotebook (run) {
  if (viewingNotebook() && notebookInstance) run(notebookInstance.run)
  else toast('That is a notebook command — open a notebook first.')
}

/** The same, for the board: something drawn on, or why nothing was. */
function onWhiteboard (act) {
  if (viewingWhiteboard() && whiteboardInstance) act(whiteboardInstance)
  else toast('That is a whiteboard command — open a whiteboard first.')
}

/* The palette's Recent section: what was run, newest first, capped at the
   handful the section holds. Written to config — which a relaunch reads back —
   rather than to memory, so the first palette of a new session still answers
   "the things I was just doing". */
const RECENT_COMMANDS = 6
function rememberCommand (id) {
  const rest = (state.cfg.recentCommands || []).filter((x) => x !== id)
  const recentCommands = [id, ...rest].slice(0, RECENT_COMMANDS)
  state.cfg = { ...state.cfg, recentCommands }
  api.config.set({ recentCommands })
}
...
function runCommand (id, dir = state.current?.dir || '') {
  ...
  switch (id) {
    ...
    case 'getting-started': openGettingStarted(); break
    case 'new-note': createNote(dir); break
    case 'new-flashcards': createFlashcardBank(dir); break
    ...
    case 'clip-page': clipPage(); break
    case 'open-page-in-browser': openPageInBrowser(); break
    ...
    /* ⌘T while the copilot panel is open: the menu's New Tab accelerator
       never reaches the page, so main forwards the key as this command
       instead — see the before-input-event hook in electron/main.js. */
    case 'copilot-effort': loadCopilot().then((c) => c.cycleEffort(1)).catch(() => {}); break
    ...
    case 'backup-vault':
      api.vault.backup().then((result) => {
        if (result?.ok) {
          ...
          toast(`Backed up ${result.files} files to ${result.path.split('/').pop()}`)
        }
        else if (!result?.canceled) toast(result?.error || 'The vault backup did not finish.')
      }).catch(() => toast('The vault backup did not finish.'))
      break
    case 'restore-vault':
      api.vault.restore().then((result) => {
        if (result?.ok) toast(`Restored ${result.files} files to ${result.path.split('/').pop()}`)
        else if (!result?.canceled) toast(result?.error || 'The vault restore did not finish.')
    ...
    case 'study-all':
      ensureLanguageRuntime()
        .then(({ languageStudy }) => languageStudy.open({ all: true }))
        .catch((error) => reportFailure('language study', error))
      break
    case 'study-flashcards': openFlashcardStudyAll(); break
    case 'insert-citation':
      openCitationPicker().catch((error) => reportFailure('insert citation', error))
      break
    case 'import-cards': importLanguageCsv(); break
    case 'export-cards': exportLanguageCsv(); break
    ...
    case 'expand-pane': togglePaneBelowExpand(); break
    ...
    case 'reveal':
      ...
      break
    /* The notebook's own menu. Every one of these is about a running kernel, so
       none of them means anything unless a notebook is open.

       Chosen from the menu, though, they are not the accelerator: the item was
       enabled, it was pointed at and clicked, and answering that with silence
       reads as a broken command rather than an inapplicable one. So the keys
       stay quiet — ⌘⏎ over a note has always meant nothing and still does — and
       the menu says why, the way the two export items below it already did. */
    case 'nb-run-cell': onNotebook((run) => run.cell()); break
    case 'nb-run-all': onNotebook((run) => run.all()); break
    case 'nb-run-above': onNotebook((run) => run.above()); break
    case 'nb-run-below': onNotebook((run) => run.below()); break
    /* The one with an accelerator, and so the one left silent. ⌘. is the key
       people press to call something off, and answering it over a note with a
       notice about notebooks would be noise in reply to a reflex. */
    case 'nb-interrupt': if (viewingNotebook()) notebookInstance?.run.interrupt(); break
    case 'nb-restart': onNotebook((run) => run.restart()); break
    case 'nb-restart-all': onNotebook((run) => run.restartAll()); break
    ...
    case 'export-vault': exportVaultAll(); break
    case 'import-folder': importFolderAll(); break
    ...
    case 'export-whiteboard-svg':
      ...
      break
    /* Scoped out of the palette when no board is open, so the ordinary way in
       cannot reach them — but a menu command can arrive a moment after the
       board was closed, and optional-chaining that into `undefined` answers a
       deliberate choice with nothing at all. */
    case 'whiteboard-add-note': onWhiteboard((b) => b.promptNote()); break
    case 'whiteboard-template-mind-map': onWhiteboard((b) => b.insertTemplate('mind-map')); break
    case 'whiteboard-template-study-plan': onWhiteboard((b) => b.insertTemplate('study-plan')); break
    case 'whiteboard-template-research': onWhiteboard((b) => b.insertTemplate('research')); break
  }
}
...
/* Print shows the whole article at once, and some of it only exists after it
   has been looked at: pictures finish loading, and a fenced scene draws when
   it scrolls into sight. Walking the column through its own length wakes the
   last of them, then the wait is for every image to report itself done —
   bounded, because a broken block should not hold an export hostage. */
...
/** Every note in the vault as Markdown files in a folder, for migration. */
async function exportVaultAll () {
  if (!state.vault) { toast('Open a vault first.'); return }
  setStatusRight('Exporting vault…')
  const result = await api.exportVault().catch((err) => ({ ok: false, error: err?.message }))
  setStatusRight('')
  if (result?.ok) toast(`Exported ${result.copied} ${result.copied === 1 ? 'note' : 'notes'} to ${String(result.path).split('/').pop()}`)
  else if (!result?.canceled) toast(result?.error || 'The export did not finish.')
  return result
}

/** `.md` files from a folder into the vault, for Obsidian/Notion takeout. */
async function importFolderAll () {
  if (!state.vault) { toast('Open a vault first.'); return }
  setStatusRight('Importing…')
  const result = await api.importFolder().catch((err) => ({ ok: false, error: err?.message }))
  setStatusRight('')
  if (result?.ok) {
    await loadTree()
    toast(`Imported ${result.copied} ${result.copied === 1 ? 'note' : 'notes'}` +
      `${result.skipped ? `, skipped ${result.skipped} existing` : ''}`)
  } else if (!result?.canceled) toast(result?.error || 'The import did not finish.')
  return result
}
...
async function importLanguageCsv () {
  ...
  try {
    ...
    toast(reason(err, 'The file could not be read.'))
    return
  }
  if (!picked) return

  const [{ parseSeparated, sniffDelimiter }, { importCards }] = await Promise.all([
    import('./csv.js'),
    import('./language-table.js')
  ])
...
/* The open language table as a CSV file — the round trip `importCards`
   completes. The table read is the deck's own (`exportCards`) and the CSV
   dialect is the grid's (`formatSeparated`), so the file another app gets is
   the same vocabulary the cards were built from — header and all, which is
   what an import on the other side names its columns from. */
async function exportLanguageCsv () {
  if (!state.current || !isLanguageTablePath(state.current.path)) {
    toast('Open a language table first.')
    return
  }
  const view = await ensureEditor()
  const [{ formatSeparated }, { exportCards }] = await Promise.all([
    import('./csv.js'),
    import('./language-table.js')
  ])
  const grid = exportCards(view.state.doc.toString())
  if (!grid || !grid.rows.length) {
    toast('This table has no cards to export yet.')
    return
  }
  const name = state.current.path.split('/').pop().replace(/\.[^.]+$/, '')
  let result
  try {
    result = await api.review.saveCsv(name, formatSeparated([grid.header, ...grid.rows]))
  } catch (err) {
    toast(reason(err, 'The file could not be written.'))
    return
  }
  if (!result?.ok) {
    if (result?.error) toast(reason(result, 'The file could not be written.'))
    return
  }
  toast(`Exported ${grid.rows.length} ${grid.rows.length === 1 ? 'card' : 'cards'}.`)
}
...
function paintFlashcardImage () {
  el.flashcardImagePreview.hidden = !flashcardImage
  if (flashcardImage) {
    el.flashcardImagePreviewImg.src = assetUrl(resolveHere(flashcardImage) || flashcardImage)
  } else {
    el.flashcardImagePreviewImg.removeAttribute('src')
  }
}

function setFlashcardImage (name) {
  flashcardImage = name || null
  paintFlashcardImage()
}

async function attachFlashcardImage (files) {
  const image = [...files].find((file) =>
    /^image\//i.test(file.type || '') || isImageAsset(file.name || ''))
  if (!image || !state.current) return
  try {
    const bytes = new Uint8Array(await image.arrayBuffer())
    const { name } = await api.asset.write(state.current.name, extensionFor(image), bytes)
    await loadAssets()
    setFlashcardImage(name)
    toast('Flashcard image added')
  } catch (error) {
    toast(reason(error, 'That image could not be saved into the vault.'))
  }
}
...
function renumberFlashcardChoices () {
  ...
  rows.forEach((row, index) => {
    ...
    if (input) input.placeholder = index === 0
      ? 'Correct answer'
      : `Other choice ${index}${index > 1 ? ' (optional)' : ''}`
    remove?.setAttribute('aria-label', `Remove other choice ${index}`)
  })
}

function addFlashcardChoice ({ focus = true } = {}) {
  const number = flashcardInputs().length + 1
  const row = node('div', 'flashcard-choice is-added')
  const label = node('span', '', 'Other choice')

  const input = document.createElement('input')
  input.type = 'text'
  input.placeholder = `Choice ${number} (optional)`
  input.autocomplete = 'off'

  const remove = node('button', 'flashcard-remove-choice', 'Remove')
  remove.type = 'button'
  remove.setAttribute('aria-label', `Remove other choice ${number - 1}`)
  remove.addEventListener('click', () => {
    row.remove()
    renumberFlashcardChoices()
    el.flashcardAddChoice.focus()
  })

  row.append(label, input, remove)
  el.flashcardAddChoice.before(row)
  renumberFlashcardChoices()
  if (focus) input.focus()
}

function flashcardError (message) {
  el.flashcardError.textContent = message
  el.flashcardError.hidden = !message
}

function closeFlashcardComposer ({ restore = true } = {}) {
  if (el.flashcardComposer.hidden) return
  el.flashcardComposer.hidden = true
  el.flashcardForm.reset()
  resetFlashcardChoices()
  setFlashcardImage(null)
  flashcardEditTarget = null
  flashcardError('')
  if (restore && flashcardPreviousView === 'read' && state.current) setView('read')
  if (flashcardReturnFocus instanceof HTMLElement && flashcardReturnFocus.isConnected) {
    flashcardReturnFocus.focus()
  } else if (state.current && !reading()) {
    editor?.focus()
  }
  flashcardReturnFocus = null
}

/** @param {{card: ReturnType<typeof parseFlashcards>[number], from: number,
 *  to: number, source: string} | null} editTarget
 *  @param {{ question?: string, explanation?: string } | null} [prefill]
 *  Seeds a fresh form — a PDF passage arrives as the question, its source as
 *  the explanation. Only a new card takes it: an edit target already has its
 *  text. */
function openFlashcardComposer (editTarget = null, prefill = null) {
  if (!state.current || !NOTE_EXT.test(state.current.path) || isLanguageTablePath(state.current.path)) {
    toast('Open a Markdown note or flashcard bank to add a flashcard.')
    return
  }
  if (readOnlyHere()) {
    toast('This note is read-only — unlock it before adding a flashcard.')
    return
  }

  flashcardPreviousView = state.view
  flashcardReturnFocus = document.activeElement instanceof HTMLElement
  ...
  resetFlashcardChoices()
  setFlashcardImage(null)
  const editing = editTarget?.card &&
    Number.isInteger(editTarget.from) && Number.isInteger(editTarget.to)
  flashcardEditTarget = editing
    ? { ...editTarget, path: state.current.path }
    : null
  el.flashcardTitle.textContent = editing ? 'Edit flashcard' : 'Add flashcard'
  el.flashcardAdd.textContent = editing ? 'Save changes' : 'Add flashcard'
  if (editing) {
    const options = [
      editTarget.card.options[editTarget.card.correct],
      ...editTarget.card.options.filter((_, index) => index !== editTarget.card.correct)
    ]
    while (flashcardInputs().length < options.length) {
      addFlashcardChoice({ focus: false })
    }
    el.flashcardQuestion.value = editTarget.card.question
    el.flashcardTags.value = (editTarget.card.tags || []).join(', ')
    setFlashcardImage(editTarget.card.image)
    flashcardInputs().forEach((input, index) => {
      input.value = options[index] || ''
    })
    el.flashcardExplanation.value = editTarget.card.explanation
  } else if (prefill) {
    el.flashcardQuestion.value = prefill.question || ''
    el.flashcardExplanation.value = prefill.explanation || ''
  }
  flashcardError('')
  el.flashcardComposer.hidden = false
  for (const field of [el.flashcardQuestion, el.flashcardExplanation]) sizeFlashcardTextarea(field)
  el.flashcardQuestion.focus()
}

async function addFlashcardFromForm () {
  const question = el.flashcardQuestion.value.trim()
  const answers = flashcardInputs().map((input) => input.value.trim())
  const options = answers.filter(Boolean)

  if (!question) { flashcardError('Add a question first.'); el.flashcardQuestion.focus(); return }
  if (!answers[0]) {
    flashcardError('Add the correct answer first.')
    flashcardInputs()[0]?.focus()
    return
  }
  if (options.length < 2) {
    flashcardError('Add at least one other choice.')
    flashcardInputs().find((input) => !input.value.trim())?.focus()
    return
  }
  if (!el.flashcardExplanation.value.trim()) {
    flashcardError('Add an explanation for the correct answer.')
    el.flashcardExplanation.focus()
    return
  }
  ...
  el.flashcardAdd.disabled = true
  try {
    const view = await ensureEditor()
    if (flashcardEditTarget) {
      const { from, to, source, path } = flashcardEditTarget
      if (state.current?.path !== path || view.state.doc.sliceString(from, to) !== source) {
        flashcardError('This flashcard changed while it was open. Close the form and try again.')
        return
      }
    ...
    const edited = Boolean(flashcardEditTarget)
    closeFlashcardComposer({ restore: true })
    if (viewingFlashcardBank()) paintFlashcardBank()
    toast(edited ? 'Updated flashcard.' : 'Added flashcard.')
  } catch (error) {
    console.error('flashcard insertion failed', error)
    flashcardError('The flashcard could not be added to this note.')
  } finally {
    el.flashcardAdd.disabled = false
  }
}

el.flashcardForm.addEventListener('submit', (event) => {
  event.preventDefault()
  addFlashcardFromForm()
})
el.flashcardClose.addEventListener('click', () => closeFlashcardComposer())
el.flashcardCancel.addEventListener('click', () => closeFlashcardComposer())
el.flashcardAddChoice.addEventListener('click', () => addFlashcardChoice())
for (const field of [el.flashcardQuestion, el.flashcardExplanation]) {
  field.addEventListener('input', () => sizeFlashcardTextarea(field))
}
el.flashcardImageAdd.addEventListener('click', () => el.flashcardImageInput.click())
el.flashcardImageInput.addEventListener('change', () => {
  attachFlashcardImage(el.flashcardImageInput.files || [])
  el.flashcardImageInput.value = ''
})
el.flashcardImageRemove.addEventListener('click', () => setFlashcardImage(null))
el.flashcardForm.addEventListener('paste', (event) => {
...
el.flashcardComposer.addEventListener('mousedown', (event) => {
  if (event.target === el.flashcardComposer) closeFlashcardComposer()
})
document.addEventListener('keydown', (event) => {
  if (el.flashcardComposer.hidden || event.key !== 'Escape') return
  event.preventDefault()
  closeFlashcardComposer()
})
...
function stepHistory (redo) {
  ...
  if (viewingPdf()) {
    /* Nothing to step through while the viewer is still being imported: the
       history being asked for is the one it keeps of marks made on this
       document, and it has not read the document yet. */
    if (!pdf) return
...
/* Whether `applySettings` has placed the lower pane once; before that the
   strip has not been drawn and the first call must draw it whatever it says. */
...
function applySettings (cfg) {
  ...
  /* Source files only — the editor decides that; see `setLineNumbers`. Applied
     to whichever editor exists now and again when one is built, because a
     source file opened before this ran would otherwise have no numbers until
     the setting was touched. */
  editor?.setLineNumbers(cfg.sourceNumbers === true)
  ...
  /* `outline` is an older spelling of the same preference, from when the
     sidebar chose between two panes and a boolean was enough; it still
     answers when `paneBelow` has never been set.

     Only when the answer changed: every setting comes through here, and
     picking the same pane again repaints it — the links pane by a backlink
     scan in main — for a saved search or the release of the height grip. */
  const wanted = cfg.paneBelow || (cfg.outline ? 'outline' : null)
  const resolved = PANES[wanted] && sidebarPaneAvailable(wanted) ? wanted : null
  if (!panesSettled || resolved !== state.paneBelow) setPaneBelow(wanted, false)
  else setPaneBelowHeight(savedPaneBelowHeight(), false)
  panesSettled = true
}
...
const loadSettings = () => (settingsLoading ??= Promise.all([
  import('./settings.js'),
  loadFeatureStyles('settings')
]).then(([{ mountSettings }]) => mountSettings({
    el: {
      root: el.settings,
...
const settings = {
  open: (section) => loadSettings().then((pane) => pane.open(section))
}
...
/**
 * Pin the stage across a panel slide, whichever panel is sliding.
 *
 * The sidebar toggle was the only caller for a long time, and the side pane
 * and the copilot — which shrink the same main column by the same mechanism —
 * animated their columns with nothing pinned at all. That is where the toggle
 * jank actually was: a slide over a long note handing CodeMirror a new width
 * on every frame.
 *
...
function freezePanelSlide (opening, panel) {
  ...
  const prop = PANEL_PROP.get(panel)
  if (!prop) return
  /* A reader who asked for no motion gets none — and no pinning either, since
  ...
  /* Measured before the attribute flips, which is why this runs first: the
     ...
     and the width it is about to gain is the stylesheet's own figure. */
  const width = opening
    ? parseFloat(getComputedStyle(el.app).getPropertyValue(prop)) || 0
    : panel.offsetWidth
  const mainWide = el.main.clientWidth
  const stageWide = el.stage.clientWidth
  releaseStage()
  pinStage(opening ? stageWide : stageWide + width)
  if (!width) return

  /* The slide, as a transform rather than as the grid transitioning.

     Animating `grid-template-columns` laid the shell out on every frame, and
     any layout in a frame makes the engine recompute every intersection it is
     tracking — including the one it keeps per `content-visibility: auto`
     block, of which a saved website rendered as a note has ten thousand. That
     was fifteen milliseconds a frame before a line was painted, on top of the
     pre-paint walk of the same page, and the sidebar toggle ran at eight
     frames a second on such a note whatever the tree held.

     So the grid snaps — `is-sliding` is what turns its transition off — and
     the panel and the main column are put back where they were by a
     transform, which then runs to nothing on the compositor. The main thread
     does not hear from the slide again until it ends. What moves:

     - a left panel closing: the panel keeps its width by inline style, so it
       still has a box to slide out from under the main column, and the main
       column starts a panel's width to the right and slides home;
     - a left panel opening: the main column is held at its old, wider width
       (overflowing to the right, under the panels there, which paint above
       it), starts a panel's width to the left and slides home, and the panel
       slides in beside it;
     - a right panel: the main column's left edge never moves, so only the
       panel slides — out to the right and under whatever is there when
       closing, in from there when opening, with the main column held wide
       under it until the end.

     The one reflow the slide owes — the stage to its new width, the chrome to
     its own — happens at `releaseStage`, as it did before. */
  const left = panel === el.sidebar
  if (opening) {
    el.main.style.width = `${mainWide}px`
    el.main.style.flex = 'none'
  } else {
    panel.style.width = `${width}px`
  }
  const slide = (node, from, to) => node.animate(
    [{ transform: `translateX(${from}px)` }, { transform: `translateX(${to}px)` }],
    { duration: SLIDE_MS, easing: SLIDE_EASE, fill: 'both' }
  )
  /* Where the panel's box is once the grid has snapped decides the run. A
     left panel's box stays at the left edge whether its column is there or
     not, so it slides out from nought to `-width` and in from `-width` to
     nought. A right panel's box moves with the main column's far edge: closed,
     it sits a panel's width further right than it was — off screen, or under
     the panel after it — so closing runs from `-width` (where it was) to
     nought, and opening from `+width` (where the closed box would be) to
     nought. */
  const panelRun = left
    ? (opening ? [-width, 0] : [0, -width])
...
/* How long a panel takes to arrive or leave, and the curve it does it on —
   the same figures the grid transition used, so the motion reads the same. */
const SLIDE_MS = 260
const SLIDE_EASE = 'cubic-bezier(.22, .61, .36, 1)'
const PANEL_PROP = new Map([[el.sidebar, '--rail'], [el.sidepane, '--side'], [el.aiPanel, '--chat']])
let slideAnimations = []

/**
 * Hold the sidebar's contents at one width while its column moves.
 *
 * The column narrows or widens over fifteen-odd frames, and without this every
 * ellipsised row in the tree would be re-laid out on each of them. Done by
 * inline style on the sidebar's direct children — a handful of nodes — rather
 * than by a stylesheet rule keyed on a class on `.app`, which could not be
 * expressed without restyling the whole tree (see the note in styles.css where
 * that rule used to be).
 *
 * @param {number} px  the width to hold, or 0 to let go
 */
 ...
 *  sidebar's contents are laid out for whether or not its column is open. */
const railWidth = () =>
  parseFloat(getComputedStyle(el.app).getPropertyValue('--rail')) || el.sidebar.offsetWidth
...
/* A zoom or a window resize can carry the viewport across a drawer breakpoint
   (1040px for the side pane, 760px for the sidebar). A closed pane then goes
   from a grid column to a fixed drawer in one style change, and its parked
   position is a transform with a transition on it — so the slide-out played
   for a panel nobody had opened, from on screen to off, at every ⌘+ that
   crossed the line. `resize` dispatches before that frame's style is
   recalculated, so a mark set here is in place when the media query flips;
   the drawer transitions are off under it (styles.css, narrow drawers) and
   it is lifted once that frame has painted, which leaves a real open or close
   sliding as before. */
let reflowFrame = 0
window.addEventListener('resize', () => {
  el.app.dataset.reflow = ''
  cancelAnimationFrame(reflowFrame)
  reflowFrame = requestAnimationFrame(() => {
    reflowFrame = requestAnimationFrame(() => { delete el.app.dataset.reflow })
  })
})
...
function applyTheme (id) {
  ...
  /* Already painted — and every setting passes through here, so painting it
     ...
     half of the test sees. */
  if (next === state.theme && document.documentElement.dataset.theme === resolveTheme(next)) return
  state.theme = next
...
async function attachFiles (files, insertIntoTable = /** @type {any} */ (null)) {
  ...
  for (const file of files) {
    try {
      ...
      toast(reason(err, 'That file could not be saved into the vault.'))
    }
  }
...
/* The drawn size of an orphan's thumbnail, kept in step with `.orphan-thumb` in
   styles.css. Stated on the element as well as in the sheet so the decoder
   knows what it is scaling to before the sheet is consulted. */
const ORPHAN_THUMB = { w: 56, h: 44 }
...
function openOrphansDialog (paths) {
  const rows = paths.map((path) => {
    ...
    thumb.alt = ''
    thumb.loading = 'lazy'
    /* The original at full resolution is what stands in for itself here, so a
       list of camera photographs is a list of multi-megapixel decodes. Lazy
       keeps the ones off screen from being decoded at all; `async` keeps the
       ones on screen off the thread the dialog is scrolling on, and the stated
       box lets the decoder scale down to what is actually drawn instead of
       building the full bitmap first. */
    thumb.decoding = 'async'
    thumb.width = ORPHAN_THUMB.w
    thumb.height = ORPHAN_THUMB.h
...
/* Two things that failed are two things to say. The old toast wrote over
   whatever was still on screen, so a pair of failures a moment apart — which is
   what a bulk operation produces — read as one, and the first was never seen at
   all. Queued instead, oldest first.

   `role="status"` on the element is what makes any of this reach a screen
   reader; without it every confirmation and every error in the app was
...
function toast (message) {
  ...
  if (!text) return
  /* The same thing said twice running is one thing said once — a repaint that
     re-reports the same failure should not make the reader wait twice. */
  if (toastQueue[toastQueue.length - 1] === text) return
  if (el.toast.hidden === false && el.toast.textContent === text) return
...
function showNextToast () {
  ...
  toastTimer = setTimeout(showNextToast, TOAST_MS)
}
...
function paintBootReady () {
  ...
  /* The shortcuts written into index.html are the ⌘ chords; on Windows they
     name keys the reader does not have. Once, after the page is built, and a
...
async function connectVault () {
  ...
  try {
    ...
    else toast(reason(error, 'That vault could not be opened.'))
  }
}
...
el.outlineFoldAll.addEventListener('click', toggleAllOutline)
...
function showImageContextMenu (path, event) {
  if (!path) return
  renderContextMenu([
    { label: revealLabel(), run: () => api.file.reveal(path) },
    { sep: true },
    {
...
/* The Info pane, with the handful of things it needs from here. `editorNow` is
   a getter rather than the editor itself: there may be none yet — the pane is
   painted on the way into every note, and a launch into the reading view has
   not built one. */
initInfoPane({
  el,
  api,
  ...
  isNote: (path) => NOTE_EXT.test(path),
  systemPath,
  copyPaths
})
...
initSidePane({
  ...
  /* Called immediately before the column changes, because the width being lost
     ...
     which is why this is handed in rather than reached for. */
  willSlide: (opening) => freezePanelSlide(opening, el.sidepane),
  ...fragmentRouting
})
...
api.on('dictionary:changed', () => {
  spellingKeys = ''
  /* A word taught to the dictionary, or one taken back out of it, is the one
     thing that makes a remembered verdict wrong — so they all go, and the pass
     below asks again. */
  spellKnown.clear()
  spellingDoc = null
  renderSpelling()
...
/**
 * A file a page offered, on its way into the vault.
 *
 * Downloads land in the vault's own `Downloads` folder (see `watchDownloads`
 * in electron/main.js) rather than in the desktop's, which is the whole point
 * of them arriving here: the paper you just downloaded is a row in the tree
 * and a hit in the next search, not something to go and fetch afterwards.
 *
 * Progress goes in the status line, where a long download can be watched
 * without covering anything; the ending is a toast, because by then the reader
 * has almost certainly moved on and stopped looking at the status line.
 */
api.on('web:download', async (report) => {
  ...
  if (report.state === 'started') { setStatusRight(`Downloading ${report.name}…`); return }
  if (report.state === 'progress') {
    // A server that will not say how big the file is gets the honest form:
    // how much has arrived, and no percentage invented to go with it.
    setStatusRight(report.total > 0
      ? `Downloading ${report.name} — ${Math.round((report.received / report.total) * 100)}%`
      : `Downloading ${report.name} — ${size(report.received)}`)
    return
  }
  if (report.state === 'done') {
    setStatusRight('')
    toast(`Saved ${report.name} to Downloads`)
    // The row is in the vault now, so the tree should be showing it.
    await loadTree()
    return
  }
  setStatusRight('')
  toast(report.state === 'cancelled'
    ? `Download of ${report.name} was cancelled`
    : `${report.name} could not be downloaded`)
})
...
api.on('vault:changed', async ({ paths = [] } = {}) => {
  ...
  /* Something moved on disk. If it was the open note — a link rewrite, an edit
     ...
     Only when it was *this* note, though. `reloadCurrent` reads the whole open
     file back over IPC and compares it, and this handler fires for every change
     anywhere in the vault — a sync client working through another folder, a
     copilot turn writing a run of files, a build dropping output in. Each of
     those was costing a full read of a note nobody had touched. An empty list
     is the watcher saying it does not know what moved, which is the one case
     that still has to assume the worst.

     A directory counts as this note too: a rename reaches fs.watch as the
     folder's name alone, and the note inside it moved with it. Matching only
     the exact path left the buffer stale in precisely the case this guard is
     here to catch. */
  const mine = !paths.length ||
    (open && paths.some((p) => p === open || open.startsWith(`${p}/`)))
  if (!handled && mine) await reloadCurrent()
...
api.on('vault:opened', async (vault) => {
  try {
    ...
    renderTabs()
    await loadTree()
    /* First run: an empty vault opens the Getting Started guide once, so a
       new user lands on something to read rather than an empty pane. Flag is
       per vault path in localStorage; opt-out by simply closing it. */
    try {
      const seenKey = `tulip.seenGettingStarted.${vault?.path || vault?.name || ''}`
      const hasNotes = (state.files || []).some((file) => NOTE_EXT.test(file.path))
      if (!hasNotes && !localStorage.getItem(seenKey)) {
...
window.addEventListener('keydown', (e) => {
  /* A question owns the keyboard while it is up: esc says no, and nothing else
  ...
  /* Above the overlay in the ladder because it can be opened over one — Help >
     Keyboard Shortcuts while the palette is up — and esc should take back the
     thing that arrived last. */
  if (e.key === 'Escape' && !el.shortcuts.hidden) {
    e.preventDefault()
    closeShortcuts()
    return
  }
...
function siteKeys (e) {
  ...
  /* Escape stops a page that is still coming in, which is what it means in
     every browser. Only while something is loading: over a page that has
 ...
 */
function pdfKeys (e) {
  if (!viewingPdf() || !pdf || !el.overlay.hidden) return
...
api.on('app:flush', async () => {
  ...
  let ok = true
  try {
    if (state.dirty && !await saveNow()) ok = false
    ...
    // The flashcard session owes the same barrier: its answers batch exactly
    // like the language session's.
    await fcStudy?.sink?.settle()
    await pdf?.flush()
    rememberTabs()
...
window.addEventListener('beforeunload', () => {
  ...
  /* The flashcard sink's owed answers — a reload is a close the flush barrier
     never hears about, and beforeunload cannot await the write it starts. */
  const owed = fcStudy?.sink?.owed() || []
...
// Handle for the DevTools console and ad-hoc test harnesses.
...
window.__tulip = {
  ...
  closeTab, selectTab, cycleTab, settings, toggleOutline, jumpToHeading,
  exportPdf
}
...
async function boot () {
  ...
  trapModalFocus()
  /* And the page behind one is taken out of the accessibility tree while it is
     up. The trap holds the keyboard; this holds the screen reader's own cursor,
     which does not move by Tab and was walking straight through the dialog into
     the note underneath. */
  guardModalBackground(el.app)
  ...
  state.tabs = stored.map(({ path, line, pinned, trail }) => {
    ...
    return {
      path,
      history: restored ? entries.map((entry) => ({ ...entry })) : [{ path, at: 0, top: 0, line }],
      historyAt: restored ? at : 0,
      base: null,
      pinned: !!pinned
    }
  })
  ...
  if (!state.tabs.length) {
    // Nothing stored: the note last open stands as the single tab, if there
    // is one — otherwise the strip starts empty. Only files become tabs.
    /* And in a second window, nothing at all: `lastNote` is the session's too,
       and a window opened from the Window menu opens with no tab. */
  ...
  state.tabIndex = state.tabs.length ? Math.min(Math.max(0, Number(session.tabIndex) || 0), state.tabs.length - 1) : -1
  ...
  state.tabIndex = state.tabs.length ? Math.max(0, state.tabs.indexOf(showing)) : -1
  renderTabs()
  /* Persist the strip after pruning vanished paths and repairing its index.
     This also clears stale sessions left by older builds whose config
     validator rejected a strip containing a blank (null) tab. */
  rememberTabs()
  ...
  if (opening) {
    ...
    await openNote(opening, {
      ...
      /* The app was opened, so the note was reopened: a bookmark in it is
         where the reader asked to be taken. */
      bookmark: true
    })
